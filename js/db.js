...
async function getSharedGamesForTeam(teamId, options = {}) {
    ...
    const queries = teamConstraints.flatMap((teamConstraint) => (
        dateConstraints.length > 0
            ...
            // Full-history reads intentionally use the team-only collection-group
            // indexes so legacy documents with no date field remain visible.
...
export async function rejectTeamRegistration(teamId, formId, registrationId, decisionNote = '') {
    ...
    await runTransaction(db, async (transaction) => {
        ...
        const checkoutCreationReservationId = String(registration.checkoutCreationReservationId || '').trim();
        const checkoutCreationStartedAt = registration.checkoutCreationStartedAt;
        const checkoutCreationStartedAtMs = typeof checkoutCreationStartedAt?.toMillis === 'function'
            ? checkoutCreationStartedAt.toMillis()
            : (typeof checkoutCreationStartedAt?.seconds === 'number'
                ? checkoutCreationStartedAt.seconds * 1000
                : Number(checkoutCreationStartedAt));
        const checkoutCreationReservationIsActive = checkoutCreationReservationId
            && Number.isFinite(checkoutCreationStartedAtMs)
            && Date.now() - checkoutCreationStartedAtMs < 15 * 60 * 1000;
        if (checkoutCreationReservationIsActive
...
export async function rollbackParentInviteRedemption(userId, code) {
    ...
    const cleanupFailedInviteSignup = httpsCallable(functions, 'cleanupFailedInviteSignup');
    const result = await cleanupFailedInviteSignup({
        userId,
        code: normalizedCode
    });
    const payload = result?.data || result || {};
...
function getParentRegistrationTimestampSortParts(registrationDoc, fieldPath = 'submittedAt') {
    const timestamp = registrationDoc.data()?.[fieldPath];
    if (Number.isFinite(timestamp?.seconds) && Number.isFinite(timestamp?.nanoseconds)) {
        return {
...
function hasParentRegistrationTimestamp(registrationData, fieldPath) {
    return registrationData?.[fieldPath] !== undefined
        && registrationData?.[fieldPath] !== null;
}
...
export async function listParentRegistrationApplicationsPage(userProfile = {}, options = {}) {
    ...
    queryResults.forEach((result) => {
        ...
        for (const registrationDoc of result.scannedDocs) {
            ...
            const isEligibleDocument = result.docs.includes(registrationDoc);
            if (isEligibleDocument && !isPageDocument) break;
            cursorContainer[result.definition.cursorKey] = registrationDoc;
            consumedCount += 1;
        }
...
function getDefaultTeamChatMessageConstraints(conversationId = DEFAULT_TEAM_CONVERSATION_ID) {
...
export async function getChatMessages(teamId, { limit = 50, startAfterDoc = null, conversationId = DEFAULT_TEAM_CONVERSATION_ID } = {}) {
    ...
    const defaultMessageConstraints = getDefaultTeamChatMessageConstraints(conversationId);
    let q = query(messagesRef, ...defaultMessageConstraints, orderBy('createdAt', 'desc'), limitQuery(limit));

    if (startAfterDoc) {
        q = query(messagesRef, ...defaultMessageConstraints, orderBy('createdAt', 'desc'), startAfterQuery(startAfterDoc), limitQuery(limit));
    }
...
export function subscribeToChatMessages(teamId, { limit = 50, conversationId = DEFAULT_TEAM_CONVERSATION_ID } = {}, onMessages, onError = null) {
    const messagesRef = getTeamChatMessagesRef(teamId, conversationId);
    const q = query(messagesRef, ...getDefaultTeamChatMessageConstraints(conversationId), orderBy('createdAt', 'desc'), limitQuery(limit));
...
export async function getUnreadChatCount(userId, teamId, options = {}) {
    ...
    if (latestMessageAt === undefined) {
        const latestMessageSnapshot = await getDocs(query(messagesRef, ...getDefaultTeamChatMessageConstraints(conversationId), orderBy('createdAt', 'desc'), limit(1)));
    ...
    const unreadConstraints = getDefaultTeamChatMessageConstraints(conversationId);
