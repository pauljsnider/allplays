...
runTest('should return false if didNotPlay is true, regardless of other fields', () => {
    ...
    if (result !== false) throw new Error('Test Case 1 failed: didNotPlay true, unused status');
    ...
    if (result !== false) throw new Error('Test Case 2 failed: didNotPlay true, participated true');

    // Case 3: didNotPlay true, timeMs > 0 (should still be false)
    result = hasPlayerProfileParticipation({ didNotPlay: true, timeMs: 1000 });
    if (result !== false) throw new Error('Test Case 3 failed: didNotPlay true, timeMs > 0');
    ...
    if (result !== false) throw new Error('Test Case 4 failed: didNotPlay true, with stats');
});

runTest('should return true if participated is true', () => {
    const result = hasPlayerProfileParticipation({ participated: true });
    if (result !== true) throw new Error('Test Case 5 failed: participated true');
});
...
runTest('should return true if participationStatus is "appeared"', () => {
    ...
    if (result !== true) throw new Error('Test Case 6 failed: participationStatus appeared');
});

runTest('should return true if timeMs > 0', () => {
    const result = hasPlayerProfileParticipation({ timeMs: 100 });
    if (result !== true) throw new Error('Test Case 7 failed: timeMs > 0');
});
...
runTest('should return true if any stat value is non-zero', () => {
    ...
    if (result !== true) throw new Error('Test Case 8 failed: non-zero stat');
});

runTest('should return false for empty statData (no participation)', () => {
    const result = hasPlayerProfileParticipation({});
    if (result !== false) throw new Error('Test Case 9 failed: empty statData');
});
...
runTest('should return false if no explicit participation and no stats', () => {
    ...
    if (result !== false) throw new Error('Test Case 10 failed: no explicit participation');
});
...
runTest('should return true if participationStatus is "unused" but didNotPlay is not true and other participation exists (post-fix behavior)', () => {
    ...
    if (result !== true) throw new Error('Test Case 11 failed: unused status with timeMs > 0 (expected true after fix)');

    result = hasPlayerProfileParticipation({ participationStatus: 'unused', participated: true });
    if (result !== true) throw new Error('Test Case 12 failed: unused status with participated: true (expected true after fix)');

    result = hasPlayerProfileParticipation({ participationStatus: 'unused', stats: { points: 1 } });
    if (result !== true) throw new Error('Test Case 13 failed: unused status with stats (expected true after fix)');
});
...
runTest('should return false if participationStatus is "unused" and no other participation exists', () => {
    ...
    if (result !== false) throw new Error('Test Case 14 failed: unused status with no other participation');
});
