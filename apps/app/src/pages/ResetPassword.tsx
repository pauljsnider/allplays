...
export function ResetPassword() {
  ...
  const verificationSuccessRoute = requestedNext ? `/verify-pending?next=${encodeURIComponent(requestedNext)}` : '/verify-pending';
  const successRoute = mode === 'verifyEmail' ? verificationSuccessRoute : requestedNext || '/auth';
      ...
      <div className="flex items-start gap-3">
        ...
        <div>
          ...
          <p className="mt-1 text-sm leading-6 font-semibold text-gray-600">
            Complete password reset, email verification, or account recovery.
          </p>
        </div>
      </div>
      ...
      {state === 'success' ? (
        <>
          ...
          {documentRequestedNext ? (
            <DocumentAuthLink route={documentRequestedNext} className="primary-button mt-4 w-full justify-center">
              {successLabel}
            </DocumentAuthLink>
          ) : (
            <Link to={successRoute} className="primary-button mt-4 w-full justify-center">
              {successLabel}
            </Link>
          )}
        </>
