...
const authServiceMocks = vi.hoisted(() => ({
  ...
  normalizeAuthEmail: (value: string | null | undefined) =>
    String(value || '')
      .trim()
      .toLowerCase(),
...
describe('AuthPage native post-login routing', () => {
  ...
  it('preserves the static Diamond viewer through a new email user verification handoff', async () => {
    ...
    await waitFor(() =>
      expect(authServiceMocks.signUpWithEmail).toHaveBeenCalledWith('viewer@example.com', 'password123', 'ABCD1234', diamondViewerRoute)
    );
  ...
  it.each([
    ...
    await waitFor(() =>
      expect(authServiceMocks.signUpWithEmail).toHaveBeenCalledWith('parent@example.com', 'password123', 'ABCD1234', nextRoute)
    );
    await waitFor(() => expect(router.state.location.pathname).toBe('/verify-pending'));
  ...
  it('drops an unsafe next route from the email signup and verification handoff', async () => {
    ...
    expect(authServiceMocks.signUpWithEmail.mock.calls[0]).toEqual(['parent@example.com', 'password123', 'ABCD1234']);
    await waitFor(() => expect(router.state.location.pathname).toBe('/verify-pending'));
