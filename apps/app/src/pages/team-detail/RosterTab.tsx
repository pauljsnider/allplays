...
import {
  ...
  archiveTeamTrackingItemForApp,
  createRosterParentInviteForApp,
  deactivateRosterPlayerForApp,
  loadRosterFieldDefinitionsForApp,
  reactivateRosterPlayerForApp,
  saveTeamTrackingItemForApp,
  setPlayerTrackingStatusForApp,
  type CreateRosterParentInviteForAppResult,
  type TeamDetailModel,
  type TeamDetailPlayer,
  type TeamRosterFieldDefinition,
  type TeamRosterParentInviteSummary,
  type TeamTrackingAdminItem
...
export function RosterTab({
  ...
  return (
    <section className="app-card p-4">
      <div className="flex items-center justify-between gap-3">
        <div>
          ...
          <div className="mt-0.5 text-xs font-semibold text-gray-500">
            Player photos, numbers, linked-player shortcuts, and profile drill-in.
          </div>
        </div>
      ...
      {status ? (
        <div
          className={`mt-3 rounded-xl border p-3 text-xs font-semibold ${status.success ? 'border-emerald-200 bg-emerald-50 text-emerald-700' : 'border-rose-200 bg-rose-50 text-rose-700'}`}
        >
          {status.message}
        </div>
      ) : null}
      {model.canManageTeam && rosterInviteLoading ? (
        <div className="mt-3 text-xs font-semibold text-gray-500">Loading parent invite status…</div>
      ) : null}
        ...
        {activePlayers.length ? (
          visibleActivePlayers.map((player) => (
            <PlayerRow
              ...
              pending={pendingPlayerId === player.id}
              onToggleActive={togglePlayerActiveState}
              inviteSummary={rosterInviteSummaries[player.id]}
      ...
      {activePlayerWindow.hasMore ? (
        <button
          type="button"
          className="secondary-button mt-3 !min-h-9 text-xs"
          onClick={() => setActivePlayerLimit(activePlayerWindow.nextLimit)}
        >
        ...
        <div className="mt-4 border-t border-gray-200 pt-4">
          <div className="flex items-center justify-between gap-3">
            <div>
              ...
              <div className="mt-0.5 text-xs font-semibold text-gray-500">
                Inactive players stay attached to history and can be restored anytime.
              </div>
            </div>
          ...
          <div className="mt-3 grid gap-2 sm:grid-cols-2">
            {visibleInactivePlayers.map((player) => (
              <PlayerRow
                key={player.id}
                teamId={model.team.id}
                teamName={model.team.name}
          ...
          {inactivePlayerWindow.hasMore ? (
            <button
              type="button"
              className="secondary-button mt-3 !min-h-9 text-xs"
              onClick={() => setInactivePlayerLimit(inactivePlayerWindow.nextLimit)}
            >
...
function RosterAiChatLauncher({ teamId, teamName }: { teamId: string; teamName: string }) {
  ...
  return (
    <div className="to-primary-50 mt-3 overflow-hidden rounded-2xl border border-violet-100 bg-gradient-to-br from-violet-50 via-white p-3 sm:p-4">
        ...
        <div className="flex min-w-0 items-start gap-3">
          ...
          <div className="min-w-0">
            ...
            <div className="mt-2 text-[11px] font-bold text-violet-700">
              Editable review first · invitations send only after you reply yes
            </div>
          </div>
        </div>
...
function AddPlayerCard({ teamId, authUser, onCreated }: { teamId: string; authUser: AuthState['user']; onCreated: () => Promise<void> }) {
  ...
  return (
    <div className="border-primary-100 bg-primary-50 mt-3 rounded-xl border p-3">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
      ...
      {status ? (
        <div
          className={`mt-3 text-xs font-black ${status.success ? 'text-emerald-700' : 'text-rose-700'}`}
          role="status"
          aria-live="polite"
          aria-atomic="true"
        >
      ...
      {open ? (
        <form className="mt-3 space-y-3" onSubmit={submit}>
          <label className="block">
            <span className="text-primary-700 text-[11px] font-black tracking-[0.04em] uppercase">Name</span>
            <input
              type="text"
              value={name}
              onChange={(event) => setName(event.target.value)}
              className="border-primary-200 focus:border-primary-500 focus:ring-primary-100 mt-2 min-h-10 w-full rounded-xl border bg-white px-3 text-sm font-semibold text-gray-950 outline-none focus:ring-2"
          ...
          <label className="block">
            <span className="text-primary-700 text-[11px] font-black tracking-[0.04em] uppercase">Jersey number</span>
            <input
              type="text"
              inputMode="numeric"
              enterKeyHint="next"
          ...
          <label className="block">
            <span className="text-primary-700 text-[11px] font-black tracking-[0.04em] uppercase">Photo</span>
            <input
              type="file"
              accept="image/*"
              className="file:text-primary-700 mt-2 block w-full text-sm font-semibold text-gray-600 file:mr-4 file:rounded-xl file:border-0 file:bg-white file:px-3 file:py-2 file:text-sm file:font-black"
          ...
          {fields.map((field) => (
            <RosterFieldInput
              key={field.key}
              field={field}
              value={rosterFieldValues[field.key]}
              disabled={submitting}
              onChange={(value) => setRosterFieldValues((current) => ({ ...current, [field.key]: value }))}
            />
          ))}
...
function RosterFieldInput({
  ...
  if (field.type === 'checkbox') {
    return (
      <label className="border-primary-100 flex items-start gap-3 rounded-xl border bg-white px-3 py-2">
        <input
          type="checkbox"
  ...
  return (
    <label className="block">
      <span className="text-primary-700 text-[11px] font-black tracking-[0.04em] uppercase">
        {field.label}
        {field.required ? ' *' : ''}
      </span>
      {field.type === 'menu' ? (
...
function TrackingAdminCard({
  ...
  async function submitItem(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      await saveTeamTrackingItemForApp(
        ...
        editingItemId ? { itemId: editingItemId } : undefined
      );
  ...
  return (
    <div className="border-primary-100 bg-primary-50 mt-4 rounded-xl border p-3">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <div className="text-sm font-black text-gray-950">Tracking items</div>
          <div className="mt-1 text-xs font-semibold text-gray-600">
            Manage legacy-compatible checklist items and each active player&apos;s completion status without leaving the app.
          </div>
        </div>
      ...
      {statusMessage ? (
        <div
          className={`mt-3 text-xs font-black ${statusMessage.success ? 'text-emerald-700' : 'text-rose-700'}`}
          role="status"
          aria-live="polite"
          aria-atomic="true"
        >
      ...
      <form className="mt-3 space-y-3 rounded-xl border border-white/80 bg-white p-3" onSubmit={submitItem}>
        ...
        <label className="block">
          ...
          <input
            ...
            onChange={(event) => setName(event.target.value)}
            className="border-primary-200 focus:border-primary-500 focus:ring-primary-100 mt-2 min-h-10 w-full rounded-xl border bg-white px-3 text-sm font-semibold text-gray-950 outline-none focus:ring-2"
            placeholder="Medical release form"
            disabled={submitting}
            required
          />
        </label>
        <label className="block">
          <span className="text-primary-700 text-[11px] font-black tracking-[0.04em] uppercase">Description</span>
          <textarea
            value={description}
            onChange={(event) => setDescription(event.target.value)}
            className="border-primary-200 focus:border-primary-500 focus:ring-primary-100 mt-2 min-h-24 w-full rounded-xl border bg-white px-3 py-2 text-sm font-semibold text-gray-950 outline-none focus:ring-2"
        ...
        <div className="grid gap-3 sm:grid-cols-2">
          <label className="block">
            <span className="text-primary-700 text-[11px] font-black tracking-[0.04em] uppercase">Visibility</span>
            <select
              value={visibility}
          ...
          <label className="block">
            <span className="text-primary-700 text-[11px] font-black tracking-[0.04em] uppercase">Status</span>
            <select
              value={status}
              onChange={(event) => setStatus(event.target.value === 'archived' ? 'archived' : 'active')}
              className="border-primary-200 focus:border-primary-500 focus:ring-primary-100 mt-2 min-h-10 w-full rounded-xl border bg-white px-3 text-sm font-semibold text-gray-950 outline-none focus:ring-2"
              disabled={submitting}
            >
        ...
        <div className="flex flex-wrap gap-2">
          ...
          {editingItemId ? (
            <button type="button" className="secondary-button !min-h-10 text-xs" onClick={resetForm} disabled={submitting}>
              Reset
            </button>
          ) : null}
        </div>
      ...
      <div className="mt-3 space-y-3">
        {visibleItems.length ? (
          visibleItems.map((item) => {
            ...
            const statusWindow = calculateRosterRenderWindow(
              item.playerStatuses.length,
              statusRowsExpanded ? (statusLimits[item.id] ?? rosterRenderLimits.trackingStatuses) : 0,
              rosterRenderLimits.trackingStatuses
            );
              ...
              <div key={item.id} className="rounded-xl border border-white/80 bg-white p-3">
                <div className="flex flex-wrap items-start justify-between gap-2">
                  <div>
                    ...
                    <div className="mt-2 flex flex-wrap gap-2">
                      <span className="bg-primary-50 text-primary-700 rounded-full px-2 py-0.5 text-[10px] font-black tracking-[0.04em] uppercase">
                        {item.visibility}
                      </span>
                      <span
                        className={`rounded-full px-2 py-0.5 text-[10px] font-black tracking-[0.04em] uppercase ${item.status === 'archived' ? 'bg-gray-100 text-gray-700' : 'bg-emerald-50 text-emerald-700'}`}
                      >
                        {item.status}
                      </span>
                      <span className="rounded-full bg-gray-100 px-2 py-0.5 text-[10px] font-black text-gray-700">
                        {item.completionSummary.complete}/{item.completionSummary.total} done
                      </span>
                    </div>
                  </div>
                  <div className="flex flex-wrap gap-2">
                    <button
                      type="button"
                      className="secondary-button !min-h-8 text-xs"
                      onClick={() => toggleStatusRows(item.id)}
                      aria-expanded={statusRowsExpanded}
                      aria-controls={`tracking-statuses-${item.id}`}
                      aria-label={`${statusRowsExpanded ? 'Hide players' : `Show players (${item.playerStatuses.length})`} for ${itemName}`}
                    >
                    ...
                    <button
                      ...
                      disabled={submitting || Boolean(busyKey)}
                    >
                      Edit
                    </button>
                    {item.status === 'active' ? (
                      <button
                        type="button"
                        className="secondary-button !min-h-8 !border-rose-200 !bg-rose-50 text-xs !text-rose-700"
                ...
                {statusRowsExpanded ? (
                  <div id={`tracking-statuses-${item.id}`} className="mt-3 grid gap-2 sm:grid-cols-2">
                    {item.playerStatuses.length ? (
                      visibleStatuses.map((playerStatus) => (
                        <div
                          key={`${item.id}:${playerStatus.playerId}`}
                          data-testid="tracking-status-row"
                        ...
                        >
                          <div className="flex min-w-0 items-center gap-3">
                            ...
                            <div className="min-w-0">
                              <div className="truncate text-xs font-black text-gray-950">
                                {playerStatus.playerNumber ? `#${playerStatus.playerNumber} ` : ''}
                          ...
                          <button
                            type="button"
                            className={`secondary-button !min-h-8 text-xs ${playerStatus.complete ? '!border-emerald-200 !bg-emerald-50 !text-emerald-700' : '!border-amber-200 !bg-amber-50 !text-amber-700'}`}
                            onClick={() => void togglePlayerStatus(item, playerStatus.playerId, playerStatus.complete)}
                            disabled={Boolean(busyKey)}
                          >
                            {busyKey === `status:${item.id}:${playerStatus.playerId}` ? (
                      ...
                      <div className="rounded-xl border border-dashed border-gray-300 bg-gray-50 p-3 text-xs font-semibold text-gray-500">
                        Add active roster players to manage statuses here.
                      </div>
                    )}
                    {statusWindow.hasMore ? (
                      <button
                        type="button"
                        className="secondary-button !min-h-9 text-xs sm:col-span-2"
                        onClick={() => setStatusLimits((current) => ({ ...current, [item.id]: statusWindow.nextLimit }))}
                        aria-label={`Show ${Math.min(rosterRenderLimits.trackingStatuses, statusWindow.hiddenCount)} more statuses for ${itemName}`}
                      >
...
function PlayerRow({
  ...
  return (
    <div
      ...
      className="hover:border-primary-200 hover:bg-primary-50/40 cursor-pointer rounded-xl border border-gray-200 bg-gray-50 p-3 transition"
      onClick={(event) => openPlayerFromRow(event.target)}
    >
      <div className="flex min-w-0 items-center gap-3">
        <Link to={playerPath} className="hover:text-primary-700 flex min-w-0 flex-1 items-center gap-3 transition">
          ...
          <span className="min-w-0 flex-1">
            <span className="flex min-w-0 items-center gap-2">
              ...
              {player.isLinked ? (
                <span className="bg-primary-600 rounded-full px-2 py-0.5 text-[10px] font-black text-white">Yours</span>
              ) : null}
              {!player.active ? (
                <span className="rounded-full bg-gray-200 px-2 py-0.5 text-[10px] font-black text-gray-700">Inactive</span>
              ) : null}
            </span>
          ...
          {parentContacts.map((contact, index) => {
            ...
            return (
              <div key={key} className="flex flex-wrap items-center gap-x-2 gap-y-1 text-xs">
                ...
                <span className="rounded-full bg-gray-100 px-2 py-0.5 text-[10px] font-black tracking-[0.04em] text-gray-600 uppercase">
                  {relation}
                </span>
                {contact.email ? (
                  <a className="text-primary-700 font-semibold" href={`mailto:${contact.email}`}>
                    {contact.email}
                  </a>
                ) : null}
                {contact.phone ? (
                  <a className="text-primary-700 font-semibold" href={`tel:${contact.phone}`}>
                    {contact.phone}
                  </a>
                ) : null}
              </div>
      ...
      {canManageTeam ? (
        <div className="mt-3 rounded-lg border border-white/80 bg-white p-3">
          <div className="flex flex-wrap items-center justify-between gap-2">
            <span className={`rounded-full px-2.5 py-1 text-[10px] font-black tracking-[0.04em] uppercase ${statusClassName}`}>
              {statusLabel}
            </span>
            {player.active ? (
          ...
          {player.active ? (
            <div className="mt-3 grid gap-2 sm:grid-cols-[minmax(0,1fr)_10rem]">
              ...
              <label className="text-xs font-black text-gray-700">
                ...
                <select
                  className="auth-input mt-1"
                  aria-label={`Parent relation for ${player.name}`}
              ...
              <div className="text-[11px] font-semibold text-gray-500 sm:col-span-2">
                Enter an email to send the invite, or leave it blank for a shareable link and code.
              </div>
            </div>
          ) : null}
          {inviteSummary?.status === 'accepted' && inviteSummary.acceptedParentCount > 0 ? (
            <div className="mt-2 text-xs font-semibold text-emerald-700">
              {inviteSummary.acceptedParentCount} linked parent{inviteSummary.acceptedParentCount === 1 ? '' : 's'}.
            </div>
          ) : null}
          {inviteSummary?.status === 'pending' && inviteSummary.pendingInviteCount > 0 && !inviteResult ? (
            <div className="mt-2 text-xs font-semibold text-amber-700">
              {inviteSummary.pendingInviteCount} pending invite{inviteSummary.pendingInviteCount === 1 ? '' : 's'}.
            </div>
          ) : null}
          {!player.active ? (
            <div className="mt-2 text-xs font-semibold text-gray-500">Reactivate the player to send a parent invite.</div>
          ) : null}
          {inviteResult ? (
            <InviteResultCard
          ...
          {inviteStatus ? (
            <div className={`mt-2 text-xs font-black ${inviteStatus.success ? 'text-emerald-700' : 'text-rose-700'}`} role="status">
