...
import type {
  TeamDetailAnalytics,
  TeamDetailAnalyticsSnapshot,
  TeamDetailModel,
  TeamDetailRosterStatisticsTable
            ...
            {model.leaderboards.length ? (
              model.leaderboards.map((leaderboard) => (
                <div key={leaderboard.id} className="rounded-xl border border-gray-200 bg-white p-3">
                  ...
                  <div className="mt-0.5 text-[10px] font-bold tracking-wide text-gray-500 uppercase">
...
function RosterStatisticsCard({
  ...
  const publicProjectionStatus =
    table?.diamond?.publicStatsStatus === 'complete'
      ? 'Complete'
  ...
  return (
    <section className="app-card p-4" aria-labelledby="roster-statistics-heading">
      <div className="flex items-start justify-between gap-3">
        <div>
          <div id="roster-statistics-heading" className="text-sm font-black text-gray-950">
            Roster statistics
          </div>
          <div className="mt-0.5 text-xs font-semibold text-gray-500">Season totals from completed tracked games.</div>
        </div>
        {!seasonUnavailable && table?.diamond?.hasDiamond ? (
          <button
            type="button"
          ...
          >
            ...
            {exporting
              ? 'Preparing CSV'
              : table.diamond.statVisibility === 'manager-internal'
                ? 'Export internal CSV'
                : 'Export public CSV'}
          </button>
        ) : null}
      ...
      <div className="mt-3">
        ...
        {!loading && !error && seasonUnavailable ? (
          <InlineDeferredError
            title="Roster statistics unavailable"
            message={`Statistics for the ${selectedSeason} season could not be loaded.`}
          />
        ) : null}
        {!loading && !error && !seasonUnavailable && table?.diamond?.hasDiamond ? (
          ...
          >
            ...
            <div className="mt-1 text-[11px] font-bold opacity-75">
              Source revisions: {table.diamond.sourceRevisions.length ? table.diamond.sourceRevisions.join(', ') : 'unavailable'}
            </div>
          ...
          <div className="mb-3 rounded-xl border border-gray-200 bg-white p-3" data-diamond-team-totals>
            ...
            <div className="mt-2 grid grid-cols-2 gap-2 sm:grid-cols-4 lg:grid-cols-6">
              {table.teamStats.columns.map((column) => {
                ...
                return (
                  <div key={column.id} className="rounded-lg bg-gray-50 px-3 py-2">
                    <div className="text-[9px] font-black tracking-wide text-gray-500 uppercase">{column.label}</div>
                    <div
                      className="mt-0.5 text-sm font-black text-gray-900"
                      aria-label={
                        value?.observed
                          ? `${formattedValue}, observed from partial tracking`
                          : formattedValue === '—'
                            ? 'Not collected'
                            : formattedValue
                      }
                    >
                      {formattedValue}
                      {value?.observed ? (
                        <span className="ml-1 text-[8px] font-black tracking-wide text-amber-700 uppercase">Observed</span>
                      ) : null}
                    </div>
                  </div>
                );
        ...
        {!loading && !error && !seasonUnavailable && table?.columns.length ? (
          <div className="overflow-x-auto rounded-xl border border-gray-200">
            <table className="w-full min-w-max text-left text-xs">
              <thead className="bg-gray-50 text-[10px] font-black tracking-wide text-gray-500 uppercase">
                <tr>
                  <th className="sticky left-0 bg-gray-50 px-3 py-2">Player</th>
              ...
              <tbody>
                {table.rows.map((row) => (
                  <tr key={row.playerId} className="border-t border-gray-100">
                    <th className="sticky left-0 bg-white px-3 py-2 font-black text-gray-900">
                      <PlayerProfileName
                        teamId={model.team.id}
                        playerId={row.playerId}
                        playerName={row.playerName}
                        playerNumber={row.playerNumber}
                        canOpenProfile={row.canOpenProfile}
                      />
                    </th>
                    {table.columns.map((column) => {
                      ...
                      return (
                        <td key={column.id} className="px-3 py-2 font-bold text-gray-700">
                          <span
                          ...
                          >
                            ...
                            {value?.observed ? (
                              <span className="text-[8px] font-black tracking-wide text-amber-700 uppercase">Observed</span>
                            ) : null}
                          </span>
                        </td>
                      );
        ...
        {!loading && !error && !seasonUnavailable && !table?.columns.length ? (
          <div className="rounded-xl border border-gray-200 bg-gray-50 p-4 text-sm font-semibold text-gray-500">
            Roster statistics appear after visible player stats are configured.
          </div>
        ) : null}
      </div>
    </section>
...
function PlayerProfileName({
  ...
  return (
    <Link
      ...
      className={`${className} hover:text-primary-700 transition`}
