...
const privateNoteLifecycles = new Set<DiamondScorebookSnapshot['lifecycle']>(['ready', 'active', 'suspended', 'final', 'correction']);
...
function privateHistoryRequestSourcesMatch(current: PrivateHistoryRequestSource | null, requested: PrivateHistoryRequestSource) {
  return Boolean(
    current &&
...
export function DiamondScorebook({
  ...
  const submitCommand = useCallback(
    async (
      ...
      const mayUseDurableOfflineQueue = type !== 'private_note' && type !== 'void_event' && type !== 'supersede_event';
      if (!networkOnline || queueCount > 0) {
  ...
  const submitPrivateMaterialCorrection = useCallback(
    async (
      ...
      if (
        ...
        !privateMaterialCorrectionSourceMatches(snapshotRef.current, privateHistoryRef.current, source, authenticatedUidRef.current)
      ) {
        setNotice({
          tone: 'error',
          message:
      ...
      try {
        ...
        if (
          ...
          source.teamId !== teamId ||
          source.gameId !== gameId ||
          !privateMaterialCorrectionSourceMatches(currentSnapshot, privateHistoryRef.current, source, authenticatedUidRef.current)
        ) {
          throw new DiamondScorebookError(
            'conflict',
            'This private correction review expired because its authorization, game instance, revision, lifecycle, or complete private-history evidence changed.'
  ...
  const correctionCandidates = useMemo(
    () =>
      (privateHistory?.items || []).filter(
        (event) =>
          event.privateMaterialStatus !== 'deleted' && !uncorrectableEventTypes.has(event.type) && !correctedEventIds.has(event.eventId)
      ),
          ...
          <section className="app-card p-3 sm:p-4">
            <div className="flex items-center justify-between gap-3">
              ...
              <button
                ...
                disabled={
                  loadingPrivateHistory || loadingOlderPrivateHistory || !networkOnline || !currentPrivateHistorySource || queueCount > 0
                }
