...
export function OpportunityManage({ auth }: { auth: AuthState }) {
  ...
  return (
    <div className="space-y-4">
      ...
      {!loading && tab === 'inquiries' ? inquiries.length || inquiryCursor ? <div className="space-y-3">{inquiries.length ? inquiries.map((inquiry) => <Link key={inquiry.id} to={`/messages?inquiry=${encodeURIComponent(inquiry.id)}`} className="app-card flex items-center gap-3 p-4"><div className="flex h-10 w-10 items-center justify-center rounded-xl bg-primary-50 text-primary-700"><Inbox className="h-5 w-5" /></div><span className="min-w-0 flex-1"><span className="block truncate text-sm font-black text-gray-950">{inquiry.listingTitle}</span><span className="mt-1 block truncate text-xs font-semibold text-gray-500">{inquiry.lastMessagePreview || `Private inquiry · ${inquiry.status}`}</span></span><span className="text-primary-700">›</span></Link>) : <Empty icon={Inbox} title="No inquiries on this page" detail="Continue loading to look past inquiries you can no longer access." />}{inquiryCursor ? <button type="button" className="ghost-button mx-auto" disabled={loadingMoreInquiries} onClick={() => void loadMoreInquiries()}>{loadingMoreInquiries ? <Loader2 className="h-4 w-4 animate-spin" /> : null}Load more inquiries</button> : null}</div> : <Empty icon={Inbox} title="No inquiries" detail="Private responses to your listings—and inquiries you send—will appear here." /> : null}
