...
export async function applyTrackStatsheetImportForApp({
  ...
  if (file && !statSheetPhotoUrl) {
    const uploaded = await uploadStatSheetPhoto(teamId, gameId, file, { returnUpload: true })
    statSheetPhotoUrl = String(uploaded?.url || '')
    if (uploaded?.path) {
      newlyUploadedPhoto = {
        url: statSheetPhotoUrl,
        path: String(uploaded.path),
        storage: uploaded.storage === 'primary' ? 'primary' : uploaded.storage === 'image' ? 'image' : undefined
      }
    }
  }
