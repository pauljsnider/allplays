...
const gameReportStatsMocks = vi.hoisted(() => ({
  resolveReportStatColumns: vi.fn(() => ({
    statKeys: [] as string[],
    statLabels: {} as Record<string, string>,
    statDefinitions: {} as Record<string, Record<string, unknown>>
  })),
  resolveOpponentReportStatColumns: vi.fn(() => ({
    oppKeys: [] as string[],
    oppLabels: {} as Record<string, string>,
    oppDefinitions: {} as Record<string, Record<string, unknown>>
  }))
}));
...
function buildCompleteManagerStatsResult(playerIds: string[], identity: ReturnType<typeof configureCurrentDiamondReportFixture>) {
  return {
...
describe('gameReportService', () => {
  beforeEach(() => {
    ...
    liveGameStateMocks.resolveLiveStatConfig.mockImplementation(
      (input?: any) => input?.configs?.[0] || { diamondPublicTeamStatIds: ['r'] }
    );
    ...
    diamondFirebaseMocks.httpsCallable.mockImplementation((_functions: unknown, name: string) =>
      name === 'getPublicDiamondGame' ? diamondFirebaseMocks.getPublicDiamondGame : diamondFirebaseMocks.getDiamondManagerStats
    );
  });

  it('accepts only bounded, cited, published Diamond AI artifacts', () => {
    expect(
      normalizePublishedDiamondAiRecap({
        schemaVersion: 1,
        trackingEngine: 'diamond-v2',
        published: true,
        status: 'current',
  ...
  it('retries one transient Diamond config failure and keeps the resolved public stats', async () => {
    ...
    dbMocks.getConfigs.mockRejectedValueOnce(new Error('temporary config read failure')).mockResolvedValueOnce([config]);
  ...
  it('retries an initially empty Diamond config read and resolves the exact config before rendering stats', async () => {
    configureCurrentDiamondReportFixture();
    const config = buildPublicDiamondConfig();
    dbMocks.getConfigs.mockResolvedValueOnce([]).mockResolvedValueOnce([config]);
    liveGameStateMocks.resolveLiveStatConfig.mockImplementationOnce(({ configs }: { configs: any[] }) => configs[0] || null);
    gameReportStatsMocks.resolveReportStatColumns.mockReturnValueOnce({
      statKeys: ['h'],
      statLabels: { h: 'H' },
  ...
  it('does not cache an unresolved empty Diamond config read as authoritative absence', async () => {
    configureCurrentDiamondReportFixture();
    const config = buildPublicDiamondConfig();
    dbMocks.getConfigs.mockResolvedValueOnce([]).mockResolvedValueOnce([]).mockResolvedValueOnce([config]);
    liveGameStateMocks.resolveLiveStatConfig.mockImplementationOnce(({ configs }: { configs: any[] }) => configs[0] || null);
    gameReportStatsMocks.resolveReportStatColumns.mockReturnValueOnce({
      statKeys: ['h'],
      statLabels: { h: 'H' },
  ...
  it('keeps activation-recorded identities and synthesizes projected players missing from the current roster and CSV', async () => {
    ...
    expect(report.playerRows.map((player) => player.playerId)).toEqual(['player-recorded', 'manual:a', 'manual:z']);
  ...
  it.each([
    ...
    dbMocks.getPlayers.mockResolvedValue(
      playerIds.map((id, index) => ({ id, name: `Current Player ${String(index + 1)}`, number: String(index + 1) }))
    );
    ...
    dbMocks.getConfigs.mockResolvedValue([buildPublicDiamondConfig()]);
    diamondManagerStatsMocks.loadDiamondManagerStats.mockImplementation(
      async ({ playerIds: requestedPlayerIds }: { playerIds: string[] }) => buildCompleteManagerStatsResult(requestedPlayerIds, identity)
    );
  ...
  it('requires every manager chunk to carry the same complete team document before applying private player stats', async () => {
    ...
    diamondManagerStatsMocks.loadDiamondManagerStats.mockImplementation(
      async ({ playerIds: requestedPlayerIds }: { playerIds: string[] }) => {
        const result = buildCompleteManagerStatsResult(requestedPlayerIds, identity);
        return requestedPlayerIds.length === 1 ? { ...result, teamDocumentsByGameId: new Map() } : result;
      }
    );
  ...
  it('retries the whole manager scope and recovers when a missing team document is transient', async () => {
    ...
    diamondManagerStatsMocks.loadDiamondManagerStats.mockImplementation(
      async ({ playerIds: requestedPlayerIds }: { playerIds: string[] }) => {
        ...
        return {
          ...result,
          teamDocumentsByGameId: new Map([['game-1', { ...buildManagerTeamDocument(identity), stats: { r: 9 } }]])
        };
      }
    );
      ...
      async ({ playerIds: requestedPlayerIds }: { playerIds: string[] }) => {
        ...
        if (mode === 'duplicate-player') {
          return {
            ...result,
            documentsByGameId: new Map([
              ['game-1', [{ id: 'player-01', data: buildManagerPlayerDocument({ playerId: 'player-01', ...identity }) }]]
            ])
          };
        }
        return {
          ...result,
          teamDocumentsByGameId: new Map([['game-1', { ...buildManagerTeamDocument(identity), stats: { r: 2 } }]])
        };
      }
    );
  ...
  it('normalizes malformed and partial Firestore stats payloads at the mapper boundary', async () => {
    ...
    expect(gameReportStatsMocks.resolveOpponentReportStatColumns).toHaveBeenCalledWith(
  ...
  it('preserves Diamond complete, observed, and unavailable evidence with source revisions', async () => {
    ...
    expect(firebaseMocks.collection).not.toHaveBeenCalledWith(firebaseMocks.db, expect.stringContaining('/teamStats'));
    ...
    expect(diamondFirebaseMocks.httpsCallable).toHaveBeenCalledWith(diamondFirebaseMocks.functions, 'getPublicDiamondGame');
  ...
  it('rejects incomplete Diamond replay pages instead of reporting authoritative zero plays', async () => {
    ...
    await expect(loadGameReportSections('team-1', 'game-1')).rejects.toThrow('Diamond public replay is incomplete');
    expect(dbMocks.getGameEvents).not.toHaveBeenCalled();
  });
        ...
        items: [
          {
            ...
            type: 'activate',
            payload: {},
            createdAt: '2026-09-05T20:00:01.000Z',
            voidsEventId: null,
            supersedesEventId: null
          }
        ]
  ...
  it('applies canonical correction semantics, removes dependent voided attachments and private notes, and keeps revision order', async () => {
      ...
      items: [
        {
          ...
          type: 'activate',
          payload: {},
          createdAt: '2026-09-05T20:09:00.000Z',
          voidsEventId: null,
          supersedesEventId: null
        },
        {
          ...
          type: 'start',
          payload: {},
          createdAt: '2026-09-05T20:08:00.000Z',
          voidsEventId: null,
          supersedesEventId: null
        },
        {
        ...
        },
        {
          ...
          payload: { playEventId: 'event-3' },
          createdAt: '2026-09-05T20:06:00.000Z',
          voidsEventId: null,
          supersedesEventId: null
        },
        {
        ...
        },
        {
          ...
          revision: 6,
          type: 'void_event',
          payload: { targetEventId: 'event-3', reason: 'correction' },
          createdAt: '2026-09-05T20:04:00.000Z',
          voidsEventId: 'event-3',
          supersedesEventId: null
        },
        {
          eventId: 'event-7',
          ...
          revision: 7,
          type: 'record_plate_appearance',
          payload: { result: 'double' },
          createdAt: '2026-09-05T20:03:00.000Z',
          voidsEventId: null,
          supersedesEventId: null
        },
        {
          ...
          payload: { playEventId: 'event-7' },
          createdAt: '2026-09-05T20:02:00.000Z',
          voidsEventId: null,
          supersedesEventId: null
        },
        {
    ...
    expect(report.plays.map((play) => play.id)).not.toEqual(
      expect.arrayContaining(['event-3', 'event-4', 'event-5', 'event-6', 'event-7'])
    );
  });
      ...
      data: {
        ...
        events: [
          ...
          },
          {
            ...
            inning: 1,
            half: 'top',
            description: 'Scoring correction recorded',
            createdAt: '2026-09-05T20:06:00.000Z'
          },
          {
            id: 'event-supersede',
            ...
            inning: 1,
            half: 'top',
            description: 'Scoring correction replaced a prior play',
            createdAt: '2026-09-05T20:09:00.000Z'
          }
        ],
        nextCursor: null,
  ...
  it('rejects a transport-complete bootstrap replay from a full Diamond report', async () => {
    ...
    await expect(loadGameReportSections('team-1', 'game-1')).rejects.toThrow('Diamond public replay is not projection-current');
  });
