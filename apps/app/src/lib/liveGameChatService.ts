import {
  ...
  subscribeLiveChat
} from './adapters/legacyLiveGameChat';
import {
...
export function subscribeToLiveGameChat(
  ...
  if (options?.diamond) {
    return subscribeDiamondLiveChat<LiveGameChatMessage>(teamId, gameId, options.diamond.instanceId, callback, onError);
  }
...
export async function sendLiveGameChatMessage(
  ...
  if (input.diamond) {
    ...
    if (input.diamond.trackingEngine !== 'diamond-v2' || !UUID_V4_PATTERN.test(instanceId)) {
    ...
    const requestId = pendingDiamondChatRequests.get(pendingKey) || createDiamondLiveEngagementRequestId();
...
export async function moderateLiveGameChatMessage(
  ...
  const requestId = pendingDiamondModerationRequests.get(pendingKey) || createDiamondLiveEngagementRequestId();
  pendingDiamondModerationRequests.set(pendingKey, requestId);
  await moderateDiamondLiveChat({ teamId, gameId, instanceId, requestId }, normalizedMessageId);
