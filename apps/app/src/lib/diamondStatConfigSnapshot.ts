...
import { DIAMOND_PLAYER_STAT_CATALOG, DIAMOND_TEAM_STAT_CATALOG } from './adapters/legacyDiamondStatPresentation';
...
function isExactStatId(value: unknown): value is string {
  return typeof value === 'string' && value === value.trim() && SAFE_STAT_ID_PATTERN.test(value) && !BLOCKED_STAT_IDS.has(value);
}
...
function safePresentationOverrides(value: Record<string, unknown>) {
  ...
  (['label', 'acronym', 'group'] as const).forEach((field) => {
    ...
    if (typeof candidate === 'string' && candidate === candidate.trim() && candidate.length >= 1 && candidate.length <= 160)
...
export function buildActivationPinnedDiamondPresentationConfig(config: unknown): Record<string, unknown> | null {
  ...
  const rawDefinitionsById = new Map(
    (config.statDefinitions as Record<string, unknown>[]).map((definition) => [definition.id, definition] as const)
  );
  ...
  const safeDefinitions = fixedCatalogs.flatMap(([scope, catalog]) =>
    catalog.flatMap((fixedDefinition) => {
