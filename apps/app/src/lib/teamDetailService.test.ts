...
vi.mock('../../../../js/team-access.js', () => ({
  ...
  normalizeAdminEmailList: vi.fn((adminEmails: unknown) =>
    Array.from(
      new Set(
...
vi.mock('../../../../js/team-staff-permissions.js', () => ({
  buildTeamStaffPermissionsViewModel: vi.fn(() => ({ staff: [], pendingInvites: [], helperPermissions: [], hasAnyStaff: false }))
}));
...
describe('buildTeamAnalytics', () => {
  ...
  it('keeps season snapshots separate and honors the preferred season', () => {
    ...
    expect(analytics.seasons.map((season) => [season.seasonLabel, season.completedGameCount])).toEqual([
      ['2026', 1],
      ['2025', 1]
    ]);
  });
...
describe('createStatTrackerConfigForApp', () => {
  ...
  it('waits for a timed-out native create to finish instead of issuing a duplicate REST create', async () => {
    dbMocks.createConfig.mockImplementation(
      () =>
        new Promise((resolve) => {
...
describe('updateTeamSettingsForApp', () => {
  ...
  it('writes only the normalized link fields when saving team links', async () => {
    ...
    expect(dbMocks.updateTeam).toHaveBeenCalledWith(
      'team-1',
  ...
  it('rejects invalid livestream links before writing', async () => {
    await expect(
      updateTeamSettingsForApp('team-1', { uid: 'owner-1' } as any, {
        name: 'Bears',
...
describe('addRosterPlayerForApp native writes', () => {
  ...
  it.each([
    ...
    await expect(
      addRosterPlayerForApp('team-1', { uid: 'owner-1' } as any, {
        name: 'Sam Player',
        photoFile
      })
  ...
  it('continues an ambiguous owner create only after an authoritative roster read confirms it', async () => {
    dbMocks.getPlayers.mockResolvedValueOnce([]).mockResolvedValueOnce([{ id: 'native-player-1', name: 'Sam Player' }]);
...
describe('addRosterPlayerForApp browser photo scope', () => {
  ...
  it('creates the browser player owner before uploading and persists that same id', async () => {
    ...
    dbMocks.uploadPlayerPhoto.mockImplementationOnce(async () => {
      ...
      expect(dbMocks.applyRosterCsvImportOperations).toHaveBeenNthCalledWith(1, 'team-1', [
        expect.objectContaining({
          type: 'add',
          playerId: 'native-player-1',
    ...
    expect(dbMocks.applyRosterCsvImportOperations).toHaveBeenNthCalledWith(2, 'team-1', [
      expect.objectContaining({
        type: 'update',
        playerId: 'native-player-1',
...
function buildTeamDiamondManagerTeamData(game: ReturnType<typeof buildTeamDiamondGame>, overrides: Record<string, unknown> = {}) {
  return {
    trackingEngine: 'diamond-v2',
    complete: true,
    projectionSchemaVersion: 1,
...
function prepareBoundedDiamondManagerSeason(gameCount: number, playerCount: number, projectedPlayerCount = playerCount) {
  __resetTeamDetailBaseSnapshotCacheForTests();
  ...
  firebaseMocks.getDocs.mockImplementation(async (path: string) => {
    ...
    return teamSnapshot(
      ...players
        .slice(0, projectedPlayerCount)
        .map((player) => buildTeamDiamondPlayerDoc(game, player.id, { playerName: player.name, playerNumber: player.number }))
    );
  });
  ...
  return {
    ...
    documentsByGameId: new Map(
      requestedGames.map((game) => [
        game.id,
        documentPlayerIds.map((playerId) => ({
          id: playerId,
...
describe('Team Insights manager-stat production fan-out', () => {
  ...
  it('runs the grouped season and all-season overlap with exactly three active chunks', async () => {
    ...
    expect(calls.reduce((total: number, request: any) => total + request.games.length * (request.playerIds.length + 1), 0)).toBe(6_240);
    ...
    expect(
      insights.rosterStatistics.seasons.every(
        (season) => season.diamond?.privateStatsStatus === 'complete' && season.diamond?.statVisibility === 'manager-internal'
      )
    ).toBe(true);
    expect(
      vi
        .mocked(buildPlayerLeaderboardSnapshot)
        .mock.calls.some(([input]: any[]) => input?.seasonStatsByPlayerId?.['player-01']?.h === 1_080)
    ).toBe(true);
  });
  ...
  it('binds the 100-player, 120-label late-chunk recovery envelope to 984 calls', async () => {
    ...
    diamondManagerStatsMocks.loadDiamondManagerStats.mockImplementation(async ({ games: requestedGames, playerIds }: any) => {
      ...
      const isFirstAggregateLastChunk = requestedGames.length === 40 && requestedGames[0]?.id === games[80]?.id && signatureCall === 4;
    ...
    expect(calls.reduce((total: number, request: any) => total + request.games.length * (request.playerIds.length + 1), 0)).toBe(49_920);
  ...
  it('keeps a rate-limited manager result non-authoritative and reloadable', async () => {
    ...
    expect(
      diamondManagerStatsMocks.loadDiamondManagerStats.mock.calls
        .slice(limitedCalls)
        .every(([request]: any[]) => request.games.length === games.length)
    ).toBe(true);
...
describe('team detail bootstrap loading', () => {
  ...
  it('recovers management access from REST and refreshes the auth token only after a 401', async () => {
    ...
    authServiceMocks.getNativeAuthIdToken.mockResolvedValueOnce('cached-web-token').mockResolvedValueOnce('refreshed-web-token');
    ...
    try {
      ...
      expect(dbMocks.getPlayersWithPrivateRosterContacts).toHaveBeenCalledWith(
        'team-1',
        expect.objectContaining({
      ...
      await expect(
        addRosterPlayerForApp('team-1', { uid: 'owner-1', roles: ['coach'] } as any, {
          name: 'New Player'
        })
  ...
  it('includes later-page players when the native roster fallback is paginated', async () => {
    ...
    try {
      ...
      expect(globalThis.fetch).toHaveBeenCalledTimes(2);
      expect(String(vi.mocked(globalThis.fetch).mock.calls[1][0])).toContain('pageToken=next+page%2B%2F%3D');
    } finally {
      globalThis.fetch = previousFetch;
    }
  });
  ...
  it('keeps leaderboards aggregated across all completed games while roster statistics stay season scoped', async () => {
    ...
    const config = {
      id: 'config-1',
      columns: ['PTS'],
      statDefinitions: [{ id: 'pts', label: 'PTS', scope: 'player', visibility: 'public' }]
    };
    dbMocks.getConfigs.mockResolvedValue([config]);
    vi.mocked(selectAnalyticsConfig)
      .mockReturnValueOnce(config as any)
      .mockReturnValueOnce(config as any);
    ...
    expect(insights.rosterStatistics.seasons.map((season) => season.rows.map((row) => row.values.pts.value))).toEqual([
      [12, 0],
      [4, 0]
    ]);
    expect(buildPlayerLeaderboardSnapshot).toHaveBeenCalledWith(
      expect.objectContaining({
  ...
  it('includes a projected-only Diamond participant with a safe game-time identity in public season rows', async () => {
    ...
    firebaseMocks.getDocs.mockResolvedValue(
      teamSnapshot(buildTeamDiamondPlayerDoc(game, 'manual:guest-1', { playerName: 'Recorded\u202e Guest', playerNumber: ' 44\u0000 ' }))
    );
  ...
  it('keeps roster order, uses the latest safe recorded identity, and sorts projected-only season participants', async () => {
    ...
    vi.mocked(buildPlayerLeaderboardSnapshot).mockImplementation(
      ({ players: eligiblePlayers }: any) =>
        ({
          topStats: [
            {
              id: 'h',
    ...
    expect(
      season.rows.map(({ playerId, playerName, playerNumber, canOpenProfile }) => ({
        playerId,
        playerName,
        playerNumber,
        canOpenProfile
  ...
  it('discovers a projected-only participant before requesting complete manager season statistics', async () => {
    ...
    firebaseMocks.collection.mockImplementation((_db: unknown, path: string) => path);
    firebaseMocks.getDocs.mockImplementation(async (path: string) =>
      path.includes('/diamond-game-2/')
        ? teamSnapshot()
        : teamSnapshot(buildTeamDiamondPlayerDoc(game, 'manual:guest-1', { playerName: 'Guest Batter', playerNumber: '44' }))
    );
    diamondManagerStatsMocks.loadDiamondManagerStats.mockResolvedValue({
      status: 'complete',
      reason: null,
    ...
    expect(diamondManagerStatsMocks.loadDiamondManagerStats).toHaveBeenCalledWith(
      expect.objectContaining({
        teamId: 'team-1',
  ...
  it('retains known projected-only evidence as partial when another public game read is unavailable', async () => {
    ...
    firebaseMocks.getDocs.mockImplementation(async (path: string) => {
      ...
      return teamSnapshot(
        buildTeamDiamondPlayerDoc(gameA, 'manual:known', {
  ...
  it('unions exact-matched A/B configs without including B-private values in public aggregates', async () => {
    ...
    expect(buildPlayerLeaderboardSnapshot).not.toHaveBeenCalledWith(
      expect.objectContaining({
  ...
  it('retains a private top-stat presentation override only for a mixed-config manager view', async () => {
    ...
    const configA = {
      id: 'config-a',
      baseType: 'Baseball',
      statDefinitions: [{ id: 'h', label: 'Hits A', scope: 'player', visibility: 'public', topStat: true }]
    };
    const configB = {
      id: 'config-b',
      baseType: 'Baseball',
  ...
  it('fails closed when same-ID games require distinct pinned catalog hashes', async () => {
    ...
    diamondStatConfigSnapshotMocks.currentDiamondStatConfigMatchesActivation.mockImplementation(
      ({ game: candidateGame }: any) => candidateGame.diamondStatConfigSnapshotHash === firstGame.diamondStatConfigSnapshotHash
    );
  ...
  it('retries the whole manager attempt when a player chunk omits its required team document', async () => {
    ...
    expect(diamondManagerStatsMocks.loadDiamondManagerStats.mock.calls.map(([request]) => request.games)).toEqual([games, games]);
  ...
  it('retries a 41-game by 26-player Cartesian load and never mixes private rows after a later chunk fails', async () => {
    ...
    expect(
      diamondManagerStatsMocks.loadDiamondManagerStats.mock.calls.map(([request]) => `${request.games.length}x${request.playerIds.length}`)
    ).toEqual(['40x25', '40x1', '1x25', '1x1', '40x25', '40x1', '1x25', '1x1']);
    expect(season.columns.map(({ id }) => id)).toEqual(['h']);
    expect(season.rows).toHaveLength(26);
    expect(season.rows.every((row) => row.values.h.value === 41 && !Object.prototype.hasOwnProperty.call(row.values, 'pitches'))).toBe(
      true
    );
    expect(season.diamond).toMatchObject({
      statVisibility: 'public',
  ...
  it('loads 26-player manager statistics in bounded coherent player chunks', async () => {
    ...
    firebaseMocks.getDocs.mockResolvedValue(
      teamSnapshot(
        ...players.map((player) =>
          buildTeamDiamondPlayerDoc(game as ReturnType<typeof buildTeamDiamondGame>, player.id, {
            playerName: player.name,
            playerNumber: player.number
          })
        )
    ...
    diamondManagerStatsMocks.loadDiamondManagerStats.mockImplementation(async ({ games: requestedGames, playerIds }: any) => ({
      ...
      reason: null,
      documentsByGameId: new Map(
        requestedGames.map((requestedGame: any) => [
          requestedGame.id,
  ...
  it('marks only the season whose roster aggregation fails as unavailable', async () => {
    ...
    dbMocks.getConfigs.mockResolvedValue([
      { id: 'config-1', columns: ['PTS'], statDefinitions: [{ id: 'pts', label: 'PTS', scope: 'player', visibility: 'public' }] }
    ]);
    dbMocks.getAggregatedStatsForGames.mockImplementation(async (_teamId: string, gameIds: string[]) => {
  ...
  it('scopes privileged roster contacts to managers and never reuses them for a later non-manager view', async () => {
    ...
    expect(managerModel.players[0].parentContacts).toEqual([{ userId: 'parent-1', email: 'private@example.com', relation: 'Parent' }]);
...
describe('tracking admin helpers', () => {
      ...
      if (typeof input === 'string' && input.endsWith('/trackingItems/item-1/memberTracking')) {
        return {
          docs: [
            {
              id: 'status-1',
              data: () => ({ teamId: 'team-1', trackingItemId: 'item-1', playerId: 'player-1', status: 'complete', complete: true })
            },
            {
              id: 'status-mismatch',
              data: () => ({ teamId: 'team-1', trackingItemId: 'item-2', playerId: 'player-1', status: 'complete', complete: true })
            }
          ]
        };
      }
      if (typeof input === 'string' && input.endsWith('/trackingItems/item-2/memberTracking')) {
        return {
          docs: [
            {
              id: 'status-2',
              data: () => ({ teamId: 'team-1', trackingItemId: 'item-2', playerId: 'player-1', status: 'open', complete: false })
            }
          ]
        };
      }
  ...
  it('writes legacy-compatible tracking item docs when saving in the app', async () => {
    await saveTeamTrackingItemForApp(
      'team-1',
  ...
  it('writes per-player tracking statuses with the legacy nested payload', async () => {
    await setPlayerTrackingStatusForApp(
      'team-1',
      { uid: 'coach-1', email: 'coach@example.com' } as any,
      'item-1',
      {
    ...
    expect(dbMocks.setTeamTrackingStatus).toHaveBeenCalledWith(
      'team-1',
      'item-1',
      'player-1',
      expect.objectContaining({
...
describe('canManageTeamAdmins adminEmails parity with legacy js/team-access.js', () => {
  beforeEach(() => {
    ...
    vi.mocked(hasFullTeamAccess).mockImplementation((user: any, team: any) => {
      ...
      const normalizedEmail = String(user.email || '')
        .trim()
        .toLowerCase();
      const adminEmails = (Array.isArray(team.adminEmails) ? team.adminEmails : []).map((email: string) =>
        String(email || '')
          .trim()
          .toLowerCase()
      );
  ...
  it('allows a user listed in team.adminEmails (not owner, not isAdmin) to manage admins', async () => {
    ...
    await expect(revokeTeamAdminAccessForApp('team-1', 'someoneelse@example.com', teamAdminUser)).resolves.toBeUndefined();
  ...
  it('denies a user who is neither owner, adminEmails member, isAdmin, isPlatformAdmin, nor admin-role', async () => {
    ...
    await expect(revokeTeamAdminAccessForApp('team-1', 'teamadmin@example.com', randomUser)).rejects.toThrow(
      'You do not have permission to manage admins for this team.'
    );
...
describe('buildTeamDetailModel registration provider', () => {
  ...
  it.each([
    ...
    [
      'verified current-email team admin',
      { uid: 'admin-1', email: 'ADMIN@example.com', emailVerified: true },
      { ownerId: 'owner-1', adminEmails: ['admin@example.com'] },
      true
    ],
    [
      'unverified current-email team admin',
      { uid: 'admin-1', email: 'admin@example.com', emailVerified: false },
      { ownerId: 'owner-1', adminEmails: ['admin@example.com'] },
      false
    ],
    ...
    [
      'verified legacy email-only owner',
      { uid: 'legacy-1', email: 'legacy@example.com', emailVerified: true },
      { ownerEmail: 'legacy@example.com' },
      true
    ],
    [
      'unverified legacy email-only owner',
      { uid: 'legacy-1', email: 'legacy@example.com', emailVerified: false },
      { ownerEmail: 'legacy@example.com' },
      false
    ],
    [
      'conflicting legacy owner aliases',
      { uid: 'legacy-1', email: 'legacy@example.com', emailVerified: true },
      { ownerEmail: 'legacy@example.com', ownerEmailLower: 'other@example.com' },
      false
    ],
    [
      'stale legacy alias behind a canonical owner',
      { uid: 'legacy-1', email: 'legacy@example.com', emailVerified: true },
      { ownerId: 'owner-1', ownerEmail: 'legacy@example.com' },
      false
    ],
    [
      'parent behind a malformed canonical owner',
      { uid: 'parent-1', email: 'parent@example.com', parentTeamIds: ['team-1'] },
      { ownerId: ' owner-1 ' },
      false
    ],
    ['coach-only delegate', { uid: 'coach-1', email: 'coach@example.com', coachOf: ['team-1'] }, { ownerId: 'owner-1' }, false],
    [
      'linked-player-only user',
      { uid: 'member-1', email: 'member@example.com', parentOf: [{ teamId: 'team-1' }], parentPlayerKeys: ['team-1:player-1'] },
      { ownerId: 'owner-1' },
      false
    ],
    [
      'scorekeeper delegate',
      { uid: 'scorekeeper-1', email: 'score@example.com', scorekeeperTeamIds: ['team-1'] },
      { ownerId: 'owner-1' },
      false
    ],
    ['stream delegate', { uid: 'stream-1', email: 'stream@example.com', streamTeamIds: ['team-1'] }, { ownerId: 'owner-1' }, false],
    [
      'media and video delegate',
      {
        uid: 'media-1',
        email: 'media@example.com',
        teamMediaUploadTeamIds: ['team-1'],
        mediaUploadTeamIds: ['team-1'],
        videographerTeamIds: ['team-1']
      },
      { ownerId: 'owner-1' },
      false
    ],
    ...
    expect(isEligiblePrivateCalendarSubscriberForApp(user as any, 'team-1', { id: 'team-1', name: 'Bears', ...team })).toBe(expected);
  ...
  it('does not expose the app team id as a registration provider value', () => {
    ...
    expect(built.team.registrationProvider).toEqual([{ label: 'Provider', value: 'Sports Connect' }]);
  });
