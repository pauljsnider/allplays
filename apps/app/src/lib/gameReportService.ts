...
import { getDiamondPrivateHistoryWindow, getDiamondState, type DiamondPrivateEvent } from './diamondScorebookService';
...
async function loadReportStatConfig(
  ...
  if (!isDiamondV2Game(game)) {
    ...
    return resolvedConfig && typeof resolvedConfig === 'object' && !Array.isArray(resolvedConfig) ? resolvedConfig : null;
  }
  ...
  for (let attempt = 0; attempt < 2; attempt += 1) {
    try {
      ...
      const matchingConfigs = requiredConfigId ? configs.filter((config) => String(config?.id || '').trim() === requiredConfigId) : [];
...
function normalizePublishedAiBlock(value: unknown): GameReportAiBlock | null {
  ...
  const citations = (Array.isArray(source.citations) ? source.citations : [])
    .flatMap((entry): GameReportAiCitation[] => {
      ...
      if (!eventId || eventId.length > 128 || eventId.includes('/') || !Number.isSafeInteger(revision) || revision < 1 || seen.has(key))
        return [];
      seen.add(key);
...
function filterStatsMapByIds(statsByPlayerId: Record<string, GameReportStatsRecord>, allowedIds: ReadonlySet<string>) {
  return Object.fromEntries(
    Object.entries(statsByPlayerId || {}).map(([playerId, stats]) => [
      playerId,
...
function emptyManagerStatsResult(status: 'partial' | 'unavailable', reason: string): DiamondManagerStatsReadResult {
  return {
    status,
    reason,
...
function stableManagerDocumentFingerprint(value: unknown): string {
  ...
  return `{${Object.keys(record)
    .sort()
    .map((key) => `${JSON.stringify(key)}:${stableManagerDocumentFingerprint(record[key])}`)
    .join(',')}}`;
}
...
async function loadTeamStatView(
  ...
  stats: GameReportTeamStatsFirestoreRecord;
...
function assertCurrentDiamondPublicReplayToken(projectionToken: string, sourceRevision: number, game: GameReportGameFirestoreRecord) {
  ...
  if (
    String(game.diamondProjectionStatus || '')
      .trim()
      .toLowerCase() !== 'current' ||
...
export async function loadGameReportPlays(
  ...
  if (diamondGame) {
    try {
      ...
      return {
        game,
        plays: [...result.events].sort((left, right) => Number(left.revision) - Number(right.revision)).map(normalizePlay),
        playsFresh: true,
...
function normalizeOpponentRows(
  opponentStats: GameReportGameFirestoreRecord['opponentStats'] = {},
  game: GameReportGameFirestoreRecord
): GameReportOpponentRow[] {
...
function normalizeOpponentStatsForColumns(
  opponentStats: GameReportGameFirestoreRecord['opponentStats'] = {},
  game: GameReportGameFirestoreRecord
): Record<string, GameReportStatsRecord> {
...
function normalizeHighlightClips(teamId: string, gameId: string, game: GameReportGameFirestoreRecord): GameReportHighlightClip[] {
  return (normalizeGameRecapHighlightClips(game) || [])
    .slice(0, 8)
    .map((clip: any) => {
...
export async function loadGameReportSections(teamId: string, gameId: string, options: GameReportLoadOptions = {}): Promise<GameReportData> {
  if (!teamId || !gameId) {
    throw new Error('Team and game are required.');
  }
  ...
  const [resolvedConfig, publicAggregateResult, eventLoad] = await Promise.all([
    ...
    diamondGame ? loadAggregatedStats(teamId, gameId, game) : loadAggregatedStats(teamId, gameId, game).catch(emptyAggregatedStatsResult),
    diamondGame
  ...
  const managerLoad =
    diamondGame && requestedStatVisibility === 'manager-internal'
  ...
  const managerPlayerCatalog =
    diamondGame && appliedStatVisibility === 'manager-internal' ? getManagerDiamondStatCatalog(resolvedConfig, 'player') : null;
  ...
  const teamStatLabels = Object.fromEntries(
    teamStatFields.map((field: any) => [String(field.fieldName || '').trim(), String(field.label || field.fieldName || '').trim()])
  );
  ...
  const playerRows = reportPlayers.map((player: any) => {
    ...
    const recordedIdentity = diamondGame ? publicAggregateResult.recordedPlayerIdentityMap.get(playerId) : null;
    return {
      playerId,
  ...
  const visiblePlayerRows = playerRows.filter(
    (player) => hasPlayerProfileParticipation(player) || player.didNotPlay || recordedPlayerIds.has(player.playerId)
  );
  ...
  const playerInsightRows = Object.entries(insights.playerInsightsById || {})
    .map(([playerId, playerInsights]) => ({
      playerId,
  ...
  const publishedAiRecap = normalizedAiRecap
    ? {
        ...normalizedAiRecap,
        current: normalizedAiRecap.current && !projection.pending && projection.authoritativeRevision === normalizedAiRecap.sourceRevision
      }
    : null;
