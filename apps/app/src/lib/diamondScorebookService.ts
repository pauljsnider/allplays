...
function normalizePrivateEventPage(value: unknown): DiamondPrivateEventPage {
  ...
  const items = (Array.isArray(source.items) ? source.items : []).map((entry): DiamondPrivateEvent => {
    ...
    const privateMaterialStatus =
      event.privateMaterialStatus === undefined ? undefined : event.privateMaterialStatus === 'deleted' ? 'deleted' : null;
    ...
    if (privateMaterialStatus === 'deleted' && !['private_note', 'void_event', 'supersede_event'].includes(type)) {
      throw new DiamondScorebookError(
        'invalid-response',
        'The private scorebook history marked a public event as deleted private material.'
      );
    }
    ...
    if (privateMaterialStatus === 'deleted') {
      ...
      const replacementRecord =
        replacement && typeof replacement === 'object' && !Array.isArray(replacement) ? (replacement as Record<string, unknown>) : null;
      ...
      if (!linksAreValid || (payloadKeys.length > 0 && !publicReplacementIsValid)) {
        throw new DiamondScorebookError('invalid-response', 'The private scorebook history contained malformed deleted private material.');
      }
    }
    return {
