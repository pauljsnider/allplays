...
describe('liveGameReactionsService', () => {
  ...
  it('uses the legacy viewer gate for live, same-day, and replay decisions', () => {
    vi.mocked(adapterMocks.isViewerChatEnabled).mockReturnValueOnce(true).mockReturnValueOnce(true).mockReturnValueOnce(false);
    ...
    expect(canUseLiveGameReactions({ date: '2026-06-03T09:00:00Z' }, { isReplay: true, now: new Date('2026-06-03T12:00:00Z') })).toBe(
      false
    );

    expect(adapterMocks.isViewerChatEnabled).toHaveBeenNthCalledWith(
      1,
    ...
    expect(adapterMocks.isViewerChatEnabled).toHaveBeenNthCalledWith(
      2,
      { date: '2026-06-03T09:00:00Z', liveStatus: null },
      { now: new Date('2026-06-03T12:00:00Z') }
    );
    expect(adapterMocks.isViewerChatEnabled).toHaveBeenNthCalledWith(
      3,
      { date: '2026-06-03T09:00:00Z', liveStatus: null },
      { isReplay: true, now: new Date('2026-06-03T12:00:00Z') }
    );
  });
  ...
  it('pins Diamond subscriptions and reuses one secure request ID after an ambiguous failure', async () => {
    ...
    expect(
      subscribeToLiveGameReactions('team-1', 'game-1', callback, undefined, {
