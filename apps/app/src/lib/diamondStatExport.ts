...
function seasonPresentationFromValues(
  ...
  const statCoverage = Object.fromEntries(
    entries.map(([id, value]) => {
      const status: DiamondCoverageStatus = value.status === 'complete' || value.status === 'partial' ? value.status : 'not_collected';
      return [id, status];
    })
  );
  return {
...
function statsFromSeasonValues(values: TeamDetailRosterStatisticsTable['rows'][number]['values']) {
  return Object.fromEntries(
    Object.entries(values || {}).flatMap(([id, value]) => (value.available === true && value.value !== null ? [[id, value.value]] : []))
  );
}

export function buildDiamondTeamSeasonStatsCsv(teamName: string, table: TeamDetailRosterStatisticsTable) {
...
export async function exportDiamondTeamSeasonStatsCsv(teamName: string, table: TeamDetailRosterStatisticsTable): Promise<ExportCsvResult> {
