...
import { DIAMOND_MANAGER_STATS_MAX_GAMES, loadDiamondManagerStats, type DiamondManagerStatsReadResult } from './diamondManagerStatsService';
import {
  aggregateCoverageAwareSeasonStats,
  getDiamondCatalogDefinition,
...
import { loadParentPlayerSchedule, type ParentScheduleChild, type ParentScheduleLoadResult } from './scheduleService';
...
function isNoncanonicalDiamondStatEvidence(game: Record<string, any>) {
  return game?.isPublicProjection === true || game?.isSharedGame === true;
}

async function loadRequiredPlayerRead<T>(loader: () => Promise<T>, isValid: (value: T) => boolean, errorMessage: string): Promise<T> {
...
function buildPlayerStatConfigResolution(
  ...
  if (requiredConfigIds.length) {
    ...
    for (const configId of requiredConfigIds.sort((left, right) => (left < right ? -1 : left > right ? 1 : 0))) {
      const matchingConfigs = read.configs.filter(
        (config) => String((config as Record<string, unknown> | null)?.id || '').trim() === configId
      );
      const requiredConfig = matchingConfigs.length === 1 ? matchingConfigs[0] : null;
      const referencedGames = diamondGames.filter((game) => String(game?.statTrackerConfigId || '').trim() === configId);
      if (
        !requiredConfig ||
        ...
        return null;
      const pinnedConfig = buildActivationPinnedDiamondPresentationConfig(requiredConfig);
      if (!pinnedConfig) return null;
      configsById.set(configId, pinnedConfig);
    }
    return {
  ...
  const fallbackConfig = selectAnalyticsConfig(read.configs, sport);
  const fallbackConfigId = String(fallbackConfig?.id || '').trim();
  const rawFallbackConfigs = read.configs.filter(
    (config) => String((config as Record<string, unknown> | null)?.id || '').trim() === fallbackConfigId
  );
  if (
    !fallbackConfig ||
    rawFallbackConfigs.length !== 1 ||
    diamondGames.some(
      (game) =>
        !currentDiamondStatConfigMatchesActivation({
          teamId,
          game: { ...game, statTrackerConfigId: fallbackConfigId },
          config: rawFallbackConfigs[0]
        })
    )
  )
    return null;
  const pinnedFallbackConfig = buildActivationPinnedDiamondPresentationConfig(rawFallbackConfigs[0]);
  return pinnedFallbackConfig
    ? {
        configs: [pinnedFallbackConfig],
        configsById: new Map(),
        fallbackConfig: pinnedFallbackConfig
      }
    : null;
}
...
async function resolvePlayerStatConfigs({
  ...
  const configRequiredDiamondGames = diamondGames.filter((game) => !isNoncanonicalDiamondStatEvidence(game));
  if (diamondGames.length && !configRequiredDiamondGames.length) {
    return { configs: [], configsById: new Map<string, Record<string, any>>(), fallbackConfig: null };
  }
  const initialResolution = buildPlayerStatConfigResolution(initialRead, teamId, sport, configRequiredDiamondGames);
  if (!diamondGames.length) {
  ...
  const retryResolution = buildPlayerStatConfigResolution(retryRead, teamId, sport, configRequiredDiamondGames);
  if (!retryResolution) throw new Error(diamondStatConfigUnavailableMessage);
  return retryResolution;
}

function getPlayerStatConfigForGame(resolution: PlayerStatConfigResolution, game: Record<string, any>) {
...
function mergeDiamondStatDefinitions(resolution: PlayerStatConfigResolution, visibility: 'public' | 'manager-internal') {
  const byId = new Map<string, Record<string, any>[]>();
  resolution.configs.forEach((config) => {
    const definitions =
      visibility === 'manager-internal' ? getManagerDiamondStatCatalog(config, 'player') : getPublicDiamondStatCatalog(config, 'player');
    definitions.forEach((definition) => {
      const id = String(definition?.id || '').trim();
      if (!id) return;
      const matchingDefinitions = byId.get(id) || [];
      matchingDefinitions.push(definition);
      byId.set(id, matchingDefinitions);
    });
  });
  const presentationFields = ['label', 'acronym', 'group', 'format', 'precision', 'rankingOrder', 'topStat'] as const;
  return [...byId.entries()]
    .sort(([leftId], [rightId]) => (leftId < rightId ? -1 : leftId > rightId ? 1 : 0))
    .flatMap(([id, definitions]) => {
      const fixedDefinition = getDiamondCatalogDefinition(id, 'player');
      if (!fixedDefinition) return [];
      const sharedPresentation = Object.fromEntries(
        presentationFields.flatMap((field) => {
          const values = definitions.map((definition) => definition?.[field]);
          return values.length && values.every((value) => Object.is(value, values[0])) ? [[field, values[0]]] : [];
        })
      );
      return [
        {
...
function haveDifferentDiamondStatIdSets(
  resolution: PlayerStatConfigResolution,
  games: ReadonlyArray<Record<string, any>>,
  visibility: 'public' | 'manager-internal'
) {
  const signatures = games
    .filter((game) => !isNoncanonicalDiamondStatEvidence(game))
    .map((game) => [...getDiamondStatIdsForGame(resolution, game, visibility)].sort().join('\u0000'));
...
export function resolvePlayerDiamondStatDocument({
  ...
  if (privateLoadStatus === 'not-requested') {
    return {
      document: publicDocument,
      statVisibility: 'public' as const,
      privateStatsStatus: 'not-requested' as const,
      absenceConfirmed: false
    };
  }
  if (privateLoadStatus === 'complete' && identity && !privateDocument && Object.keys(publicDocument || {}).length === 0) {
    return {
      document: {},
...
async function mapInDiamondReadBatches<T, R>(values: readonly T[], mapper: (value: T, index: number) => Promise<R>): Promise<R[]> {
  const results: R[] = [];
...
async function loadPlayerDiamondPublicStatRead(
  ...
  if (isNoncanonicalDiamondStatEvidence(game)) {
    return { game, data: {}, publicLoadStatus: 'partial', absenceConfirmed: false };
  }
  const gameId = String(game?.id || game?.gameId || '').trim();
  const collectionPath = gameId ? getDiamondPublicPlayerStatsCollectionPath({ teamId, gameId, game }) : null;
...
function isEmptyIncompletePlayerDiamondBatch(reads: readonly PlayerDiamondPublicStatRead[]) {
  return (
    reads.every(({ data }) => Object.keys(data).length === 0) &&
    reads.some(
      ({ game, publicLoadStatus, absenceConfirmed }) =>
        !isNoncanonicalDiamondStatEvidence(game) && (publicLoadStatus !== 'complete' || absenceConfirmed !== true)
    )
  );
}
...
async function loadPlayerDiamondPublicStatBatch(
  ...
  const loadOnce = () => mapInDiamondReadBatches(games, (game) => loadPlayerDiamondPublicStatRead(teamId, playerId, game));
...
function buildParentAthleteProfileShell(
...
export async function loadParentPlayerDetail(user: AuthUser | null, teamId: string, playerId: string): Promise<ParentPlayerDetailData> {
  ...
  let linkedChild =
    findLinkedChild(schedule.children, requestedTeamId, requestedPlayerId) ||
    findLinkedParentChild(accessUser, requestedTeamId, requestedPlayerId);
  ...
  if (!linkedChild && !routeAccess.isLinkedParent && !routeAccess.isTeamStaff) {
    ...
    if (hydratedUser !== accessUser) {
      ...
      try {
        ...
        scheduleLoadError =
          schedule.isPartial === true ? 'Some schedule sources are temporarily unavailable. Recent games may be incomplete.' : null;
      } catch {
  ...
  const team = requestedTeamId === resolvedTeamId ? initialTeam : await getTeam(resolvedTeamId, { includeInactive: true });
  ...
  const [
    ...
    routeAccess.isLinkedParent || routeAccess.isTeamStaff
  ...
  const playerDoc =
    (Array.isArray(players) ? players : []).find((candidate: LegacyPlayerRecord) => candidate?.id === resolvedPlayerId) || {};
  ...
  const completedGameEvents = events
    ...
    .slice(0, 6);
  const authoritativeGames = access.isTeamStaff ? await getGames(resolvedTeamId).catch(() => []) : [];
  const authoritativeGamesById = new Map(
    (Array.isArray(authoritativeGames) ? authoritativeGames : [])
  ...
  const canonicalManagerGames = requestedManagerGames.filter((game) => !isNoncanonicalDiamondStatEvidence(game));
  const managerBatch =
    access.isTeamStaff && requestedManagerGames.length && canonicalManagerGames.length === requestedManagerGames.length
      ? await loadChunkedPlayerDiamondManagerStats({
          teamId: resolvedTeamId,
          games: canonicalManagerGames,
          playerId: resolvedPlayerId
        })
      : null;
  const loadedPublicRows = await mapInDiamondReadBatches(completedGameEvents, async (event) => {
    ...
    try {
      const statDocument = await loadPlayerStatDocument(resolvedTeamId, event.id, resolvedPlayerId, gameEvidence);
  ...
  const overviewPrivateStatsStatus: ParentPlayerStatRow['privateStatsStatus'] =
    access.isTeamStaff && requestedManagerGames.length
  ...
  const statRows = loadedPublicRows.map((row): ParentPlayerStatRow => {
    ...
    const selected = managerStatsComplete
      ...
      : {
          document: row.publicDocument,
          statVisibility: 'public' as const,
          privateStatsStatus: overviewPrivateStatsStatus,
          absenceConfirmed: false
        };
  ...
  return {
    ...
    nextEvent,
    actionCounts: {
      rsvpNeeded: upcoming.filter(
        (event) => event.isDbGame && !event.availabilityLocked && normalizeRsvpResponse(event.myRsvp) === 'not_responded'
      ).length,
    ...
    incentives: buildPlayerIncentiveData({
      ...
      statOptions,
      maxPerGameCents,
      statRows: statRows.map((row) => (row.statPresentation?.isDiamond ? { ...row, stats: row.completeStats || {} } : row))
    }),
...
export async function loadParentPlayerStatsDetail(
  ...
  return loadCachedAppData(cacheKey, () => loadParentPlayerStatsDetailUncached(user, requestedTeamId, requestedPlayerId), {
    ...
    persist: false,
    shouldCache: (value) =>
      value.summary.diamond?.requestedStatVisibility !== 'manager-internal' &&
...
async function loadParentPlayerStatsDetailUncached(user: AuthUser, teamId: string, playerId: string): Promise<ParentPlayerStatsDetailData> {
  ...
  const canonicalCompletedDiamondGames = completedDiamondGames.filter((game) => !isNoncanonicalDiamondStatEvidence(game));
  const managerBatch =
    access.isTeamStaff && completedDiamondGames.length && canonicalCompletedDiamondGames.length === completedDiamondGames.length
      ? await loadChunkedPlayerDiamondManagerStats({ teamId, games: canonicalCompletedDiamondGames, playerId })
      : null;
  const publicDiamondReads =
    managerBatch?.status === 'complete' ? [] : await loadPlayerDiamondPublicStatBatch(teamId, playerId, completedDiamondGames);
  if (
    completedDiamondGames.some((game) => !isNoncanonicalDiamondStatEvidence(game)) &&
    isEmptyIncompletePlayerDiamondBatch(publicDiamondReads)
  ) {
    throw new Error('Diamond statistics are temporarily unavailable. Refresh to retry.');
  }
  const publicDiamondReadsByGameId = new Map(
    publicDiamondReads.map((read) => [String(read.game?.id || read.game?.gameId || '').trim(), read])
  );
  const loadedPublicStatRows = await mapInDiamondReadBatches(seasonGames, async (game) => {
    ...
    try {
      const statDocument = await loadPlayerStatDocument(teamId, String(game.id || game.gameId || ''), playerId, game);
  ...
  const managerResolutions = loadedPublicStatRows
    ...
    .map(({ game, publicDocument, publicLoadStatus }) =>
      resolvePlayerDiamondStatDocument({
        playerId,
        game,
        publicDocument,
  ...
  const clearMultiGamePublicFamilyCoverage =
    !managerStatsComplete && haveDifferentDiamondStatIdSets(statConfigResolution, completedDiamondGames, 'public');
  const incompleteManagerStatus: ParentPlayerStatRow['privateStatsStatus'] =
    access.isTeamStaff && completedDiamondGames.length
      ? managerBatch?.status === 'unavailable'
        ? 'unavailable'
        : 'partial'
      : 'not-requested';
  const publicStatsStatus = summarizeDiamondPublicStatsStatus(
    loadedPublicStatRows.filter(({ game }) => isDiamondV2Game(game)).map(({ publicLoadStatus }) => publicLoadStatus),
    managerStatsComplete
  );
  const loadedSeasonStatRows = loadedPublicStatRows.map(
    ({ game, publicDocument: statDocument, publicLoadStatus, publicAbsenceConfirmed }) => {
      const selected =
        isDiamondV2Game(game) && managerStatsComplete
          ? resolvePlayerDiamondStatDocument({
              playerId,
              game,
      ...
      const gameStatDefinitions =
        appliedDiamondVisibility === 'manager-internal'
      ...
      const presentation = filterDiamondPlayerStatPresentation(readCoverageAwareStatDocument(selected.document, game), gameStatIds);
      ...
      const completeStats = presentation.isDiamond ? filterStatRecordByIds(presentation.completeValues, gameStatIds) : stats;
      return {
        game,
  ...
  const toStatRow = ({
    ...
    participated,
    timeMs
  }: (typeof loadedSeasonStatRows)[number]): ParentPlayerStatRow => ({
    event,
    stats,
    completeStats,
  ...
  const diamondGames = loadedSeasonStatRows
    .filter(
      ({ game, statDocument, publicLoadStatus, absenceConfirmed }) =>
        isDiamondV2Game(game) &&
        (Object.keys(statDocument || {}).length > 0 || (!managerStatsComplete && !(publicLoadStatus === 'complete' && absenceConfirmed)))
    )
    .map(({ game, aggregateDocument }) => ({
      game,
      documents: Object.keys(aggregateDocument || {}).length ? [{ id: playerId, data: aggregateDocument }] : []
    }));
  const legacyTotals = buildStatTotals(
    loadedSeasonStatRows.filter(({ game }) => !isDiamondV2Game(game)).map(({ event, stats, timeMs }) => ({ event, stats, timeMs }))
  );
  ...
  const diamondEventEvidenceByGameId = new Map(
    loadedSeasonStatRows
      .filter(({ game }) => isDiamondV2Game(game))
      .map(({ game, statDocument, publicLoadStatus, absenceConfirmed }) => {
        ...
        return [
          String(game.id || game.gameId || '').trim(),
          {
            ...
            complete:
              sourcePlayIds !== null &&
              (managerStatsComplete ? true : publicLoadStatus === 'complete' && (hasStatDocument || absenceConfirmed))
          }
        ] as const;
      })
  );
        ...
        ? {
            ...
            diamond: {
              ...
              absenceConfirmed: managerStatsComplete
                ...
                : publicDiamondReads.every(
                    ({ data, publicLoadStatus, absenceConfirmed }) =>
...
export async function loadParentPlayerAthleteProfile(
  ...
  teamId: string,
  playerId: string
): Promise<ParentAthleteProfileData> {
  if (!user?.uid) {
    throw new Error('Player details require a signed-in user.');
  }
...
export async function loadParentPlayerDetailWithAthleteProfile(
  user: AuthUser | null,
  teamId: string,
  playerId: string
): Promise<ParentPlayerDetailData> {
  const detail = await loadParentPlayerDetail(user, teamId, playerId);
  if (detail?.athleteProfile?.profile) {
    return detail;
  }

  const athleteProfile = await loadParentPlayerAthleteProfile(user, detail.child.teamId, detail.child.playerId).catch(
    () => detail.athleteProfile
  );
  return {
    ...detail,
...
export async function loadParentPlayerStatTotals(user: AuthUser | null, teamId: string, playerId: string): Promise<ParentPlayerStatTotals> {
  ...
  const [games, configRead] = await Promise.all([loadPlayerGameHistory(requestedTeamId), loadPlayerStatConfigsOnce(requestedTeamId)]);
  const normalizedGames = Array.isArray(games) ? games : [];
  const gameIds = normalizedGames.map((game: any) => String(game?.id || game?.gameId || '').trim()).filter(Boolean);
  ...
  const canonicalDiamondGames = diamondGames.filter((game: any) => !isNoncanonicalDiamondStatEvidence(game));
  const managerBatch =
    access.isTeamStaff && canonicalDiamondGames.length === diamondGames.length
      ? await loadChunkedPlayerDiamondManagerStats({
          teamId: requestedTeamId,
          games: canonicalDiamondGames,
          playerId: requestedPlayerId
        })
      : null;
  const publicDiamondDocuments =
    managerBatch?.status === 'complete'
  ...
  if (
    diamondGames.some((game) => !isNoncanonicalDiamondStatEvidence(game)) &&
    isEmptyIncompletePlayerDiamondBatch(publicDiamondDocuments)
  ) {
    throw new Error('Diamond statistics are temporarily unavailable. Refresh to retry.');
  }
  const managerStatsComplete = Boolean(
  ...
  const clearMultiGamePublicFamilyCoverage =
    !managerStatsComplete && haveDifferentDiamondStatIdSets(statConfigResolution, diamondGames, 'public');
  ...
  const diamondDocuments = publicDiamondDocuments.flatMap(({ game, data, publicLoadStatus, absenceConfirmed }) => {
    ...
    const filteredSelected = Object.keys(selected).length
      ...
      : selected;
    if (Object.keys(selected).length === 0 && (managerStatsComplete || (publicLoadStatus === 'complete' && absenceConfirmed))) return [];
  ...
  const statPresentation = projection.presentationByPlayerId[requestedPlayerId] || {
    ...
    statCoverage: {},
    observedStatKeys: [],
    unavailableStatKeys: (managerStatsComplete ? managerDiamondStatDefinitions : publicDiamondStatDefinitions).map((definition) =>
  ...
  return {
    ...
    statDefinitions: (managerStatsComplete ? managerDiamondStatDefinitions : publicDiamondStatDefinitions).map((definition) => ({
      ...definition
    })),
    statPresentation,
    diamond: {
...
function isCompletedGame(game: Record<string, any>) {
  ...
  return (
    ...
    liveStatus === 'final' ||
    getGameDate(game).getTime() < Date.now()
  );
}
...
function buildStatsEventFromGame(
  game: Record<string, any>,
  teamId: string,
  ...
  return {
    ...
    diamondProjectionRevision: Number.isSafeInteger(Number(game?.diamondProjectionRevision))
      ? Number(game?.diamondProjectionRevision)
      : null,
...
function getGamePlayerTimeMs(
  game: Record<string, any>,
  playerId: string,
  stats: Record<string, unknown>,
  statDocument: Record<string, any> = {}
) {
  const directTime = Number(
    statDocument?.timeMs ??
      statDocument?.minutesMs ??
      statDocument?.playingTimeMs ??
      (stats as any)?.timeMs ??
      (stats as any)?.minutesMs ??
      (stats as any)?.playingTimeMs
  );
...
function filterDiamondPlayerStatPresentation(
  ...
  const statCoverage = filterStatRecordByIds(presentation.statCoverage, allowedIds) as Record<string, DiamondCoverageStatus>;
...
function filterStatsByPlayerIds(value: Record<string, Record<string, unknown>>, allowedIds: ReadonlySet<string>) {
  return Object.fromEntries(Object.entries(value || {}).map(([playerId, stats]) => [playerId, filterStatRecordByIds(stats, allowedIds)]));
}
...
function buildPlayerTrends(rows: ParentPlayerStatRow[], allowedStatKeys?: string[]): ParentPlayerTrend[] {
  ...
  return keys
    .map((key): ParentPlayerTrend | null => {
      const earlierRows = allowedStatKeys ? earlier.filter((row) => Object.prototype.hasOwnProperty.call(row.stats || {}, key)) : earlier;
      const recentRows = allowedStatKeys ? recent.filter((row) => Object.prototype.hasOwnProperty.call(row.stats || {}, key)) : recent;
  ...
  analyticsConfigs.forEach((config) => {
    ...
    definitions
      ...
      .forEach((definition: any) => {
        const statId = String(definition?.id || '')
          .trim()
          .toLowerCase();
...
function validateSourcePlayIds(value: unknown): string[] | null {
  ...
  if (normalized.some((eventId, index) => !eventId || eventId !== value[index] || eventId.length > 128 || eventId.includes('/')))
    return null;
  ...
  const status: PlayerGameEventsLoadStatus = !diamondStatuses.length
    ...
    : diamondStatuses.every((value) => value === 'complete')
      ...
      : diamondStatuses.every((value) => value === 'unavailable')
        ...
        : 'partial';
  return {
    rows: eventGroups.map(({ row }) => row).filter((row): row is ParentPlayerGameEventRow => row !== null),
    status
  };
}
...
function isEventForPlayer(event: Record<string, any>, playerId: string) {
  ...
  if (Array.isArray(event?.players) && event.players.some((player: any) => String(player?.id || player?.playerId || player) === playerId))
    return true;
  return false;
}
...
export async function savePlayerCustomRosterFieldValues({
  ...
  const { publicProfile: existingPublicProfile, privateValues: legacyProtectedValues } = splitProtectedRosterProfileValues(
    player?.profile || {}
  );
  const nextProfile = {
  ...
  const existingPrivateRosterFields =
  ...
  await updatePlayerWithPrivateRosterProfileFields(
    teamId,
    playerId,
    {
      profile: nextProfile
  ...
  if (photoFile) {
    ...
    if (nativeRuntime) {
      const uploaded = await import('./nativeStorageUpload').then((module) =>
        module.uploadNativePlayerPhotoFile(photoFile, teamId, playerId)
      );
  ...
  if (nativeRuntime) {
    try {
      ...
      const persistenceState =
        nativePhotoPath && !isDefinitiveFirestoreWriteFailure(error)
...
export async function saveStaffPlayerRosterDetails({
  ...
  if (currentPhotoPath && currentPhotoPath !== nextPhotoPath) {
    if (nativeRuntime) {
      await import('./nativeStorageUpload')
        .then((module) => module.deleteNativePrimaryStorageFile(currentPhotoPath))
        .catch(() => undefined);
    } else {
...
export async function saveParentAthleteProfileDraft({
  ...
  const selectedSeasonKeys =
    Array.isArray(draft.selectedSeasonKeys) && draft.selectedSeasonKeys.length ? draft.selectedSeasonKeys : [seasonKey];
  ...
  try {
    ...
    const saveOptions = isNewProfile ? { profileId: workingProfileId, isNewProfile: true } : { profileId: workingProfileId };
    saved = await saveAthleteProfile(
      user!.uid,
      {
        ...draft,
        profilePhoto,
...
function normalizeAthleteProfileHighlightClipDraft(clip: AthleteProfileHighlightClipDraft, index: number) {
  ...
  const url = source === 'external' ? normalizeAthleteProfileHighlightClipUrl(rawUrl) : rawUrl;
...
function normalizeHighlightClipMediaType(mediaType: unknown, mimeType: unknown, url: string, source: 'external' | 'upload') {
  const explicit = String(mediaType || '')
    .trim()
    .toLowerCase();
...
async function cleanupUploadedAthleteProfileMedia(paths: Array<string | null | undefined>) {
  await Promise.all(
    paths.filter((path): path is string => !!path).map((path) => deleteAthleteProfileMediaByPath(path).catch(() => undefined))
  );
}
...
async function getPlayerPhotoPersistenceState(teamId: string, playerId: string, expectedPhotoPath: string) {
  ...
  try {
    ...
    return String(privateProfile?.photoPath || '').trim() === expectedPhotoPath ? ('committed' as const) : ('not-committed' as const);
  } catch {
...
async function getPlayerPhotoRemovalPersistenceState(teamId: string, playerId: string, previousPhotoPath: string) {
  try {
    ...
    return authoritativePath === previousPhotoPath ? ('not-committed' as const) : ('unknown' as const);
  } catch {
    return 'unknown' as const;
  }
}
...
function isLinkedParent(user: AuthUser | null, teamId: string, playerId: string) {
  ...
  const linkedByParentOf = (user?.parentOf || []).some(
    (entry: any) =>
      safeDecode(entry?.teamId || entry?.teamID || entry?.team_id || entry?.team) === normalizedTeamId &&
      [entry?.playerId, entry?.playerID, entry?.player_id, entry?.childId, entry?.childID, entry?.child_id].some(
        (value) => safeDecode(value) === normalizedPlayerId
      )
  );
...
function isParentLinkedToTeam(user: AuthUser | null, teamId: string) {
  ...
  const linkedByParentOf = (user?.parentOf || []).some(
    (entry: any) => safeDecode(entry?.teamId || entry?.teamID || entry?.team_id || entry?.team) === normalizedTeamId
  );
  if (linkedByParentOf) return true;
...
function normalizePlayerFamilyContacts(
  ...
  const addContact = (source: Record<string, any> | null | undefined, fallback: Record<string, any> = {}) => {
    ...
    const userId = clean(
      source.userId || source.uid || source.accountUserId || source.parentUserId || source.guardianUserId || fallback.userId
    );
    ...
    const relation =
      clean(source.relation || source.relationship || source.parentRelation || source.guardianRelation || fallback.relation) ||
      'Parent/guardian';
...
function isTeamStaffUser(user: AuthUser | null, team: LegacyTeamRecord | null) {
  ...
  return !!(
    Array.isArray(user?.coachOf) && user.coachOf.map((value) => String(value || '').trim()).includes(String(team?.id || '').trim())
  );
}
...
function buildVisibleCustomRosterFields({
  ...
  return normalizedFields
    .filter((field) => {
      ...
      return canViewRosterField(
        { id: field.key, visibility: field.visibility },
        {
...
function normalizeCustomRosterFieldInput(
  values: Record<string, unknown>,
  fields: Array<Pick<RosterFieldDefinition, 'key' | 'type'>>
): RosterProfileValues {
