...
const DIAMOND_TERMINAL_LIFECYCLES = new Set(['final', 'correction', 'completed', 'cancelled', 'canceled', 'deleted']);
...
export async function loadDiamondLiveInteractionWindow(identity: DiamondLiveInteractionWindowIdentity) {
  const expected = {
    teamId: normalizeId(identity.teamId, 'teamId'),
    gameId: normalizeId(identity.gameId, 'gameId'),
    instanceId: normalizeUuid(identity.instanceId, 'instanceId')
  ...
  const response = requireRecord(
    await callDiamondLiveEngagement('getPublicDiamondGame', {
...
function validateResponse(response: unknown, identity: ReturnType<typeof normalizeIdentity>, resultField: 'messageId' | 'reactionId') {
...
export async function postDiamondLiveChat(identity: DiamondLiveEngagementIdentity, text: string) {
...
export async function postDiamondLiveReaction(identity: DiamondLiveEngagementIdentity, type: string) {
  const normalized = normalizeIdentity(identity);
...
export async function moderateDiamondLiveChat(identity: DiamondLiveEngagementIdentity, messageId: string) {
  const normalized = normalizeIdentity(identity);
