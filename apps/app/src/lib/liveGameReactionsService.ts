...
export function canUseLiveGameReactions(
  game: LiveGameReactionEligibilityGame | null | undefined,
  options?: { isReplay?: boolean; now?: Date }
) {
...
export function getLiveGameReactionNotice(
  game: LiveGameReactionEligibilityGame | null | undefined,
  options?: { isReplay?: boolean; now?: Date }
) {
  if (options?.isReplay) {
...
export function subscribeToLiveGameReactions(
  ...
  if (options?.diamond) {
    return subscribeDiamondLiveReactions<LiveGameReaction>(teamId, gameId, options.diamond.instanceId, callback, onError);
  }
...
export async function sendLiveGameReaction(
  ...
  if (input.diamond) {
    ...
    const requestId = pendingDiamondReactionRequests.get(pendingKey) || createDiamondLiveEngagementRequestId();
