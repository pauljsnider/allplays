...
function privateNoteStateForStorage(state: DiamondGameState): DiamondGameState {
  ...
  return state.currentScorerUid === null ? state : { ...state, currentScorerUid: DIAMOND_PRIVATE_NOTE_ACTOR_REDACTION };
}

function resultForPrivateNoteStorage(result: DiamondCommandResult, event: DiamondEvent): DiamondCommandResult {
  return event.actorUid !== DIAMOND_PRIVATE_NOTE_ACTOR_REDACTION ? result : { ...result, state: event.after };
}
...
function resultForPrivateNoteResponse(
  ...
  return event.actorUid !== DIAMOND_PRIVATE_NOTE_ACTOR_REDACTION ? result : { ...result, state: { ...result.state, currentScorerUid } };
}
...
function isCanonicalPrivateNoteMaterialEvent(event: DiamondEvent | undefined) {
  ...
  return (
    (event.type === 'void_event' || event.type === 'supersede_event') && event.payload.reason === DIAMOND_PRIVATE_NOTE_REASON_TOMBSTONE
  );
}
...
export function canonicalizeDiamondPrivateNoteCommand(command: DiamondCommand, privateMaterial?: boolean): DiamondCommand {
  ...
  if (canonicalCommand.type === 'supersede_event' && canonicalCommand.payload.replacement.type === 'private_note') {
    return {
      ...canonicalCommand,
      payload: {
    ...
    const context: HistoricalPlayContext = {
      ...
      plateAppearanceResult:
        event.type === 'record_plate_appearance'
          ? (event.payload as DiamondCommandPayloadMap['record_plate_appearance']).result
          : undefined,
      advances:
        event.type === 'advance_runner'
...
export function createDiamondCommandReceipt(
  ...
  const expectedCommandHash = commandHash(command, event.actorUid === DIAMOND_PRIVATE_NOTE_ACTOR_REDACTION);
...
export function executeDiamondCommandFromCheckpoint(
  ...
  try {
    ...
    if (command.teamId !== checkpoint.teamId || command.gameId !== checkpoint.gameId) {
      throw new DiamondDomainError('game-mismatch', 'The command does not belong to this checkpoint.');
    }
    if (command.rulesProfileId !== checkpoint.rulesProfileId || command.rulesProfileVersion !== checkpoint.rulesProfileVersion) {
      throw new DiamondDomainError('rules-profile-mismatch', 'The command does not use the checkpoint rules profile.');
    }
    ...
    if (existingReceipt) {
      ...
      const sameResource = (state: DiamondGameState) =>
        state.teamId === checkpoint.teamId &&
      ...
      if (
        ...
        throw new DiamondDomainError('invalid-command-receipt', 'Receipt resource or chain does not match the checkpoint.');
      }
