...
const BASEBALL_OBR_V1 = freezeProfile({
  ...
  inningRunLimit: null,
  runAheadRules: [],
  tiebreaker: { enabled: false, startInning: 10, runnerBase: 'second', runnerSelection: 'previous-batter' },
...
const BASEBALL_NFHS_V1 = freezeProfile({
  ...
  tiebreaker: { enabled: false, startInning: 8, runnerBase: 'second', runnerSelection: 'previous-batter' },
  continuousBatting: false,
  freeSubstitution: false,
...
const BASEBALL_YOUTH_V1 = freezeProfile({
  ...
  tiebreaker: { enabled: true, startInning: 7, runnerBase: 'second', runnerSelection: 'previous-batter' },
...
const FASTPITCH_NFHS_V1 = freezeProfile({
  ...
  inningRunLimit: null,
  runAheadRules: [{ afterInning: 5, runDifferential: 10 }],
  tiebreaker: { enabled: true, startInning: 8, runnerBase: 'second', runnerSelection: 'previous-batter' },
  continuousBatting: false,
  freeSubstitution: false,
  starterReentryLimit: 1,
  allowsDh: false,
...
export const DIAMOND_RULES_PROFILES: readonly DiamondRulesProfile[] = Object.freeze([
  BASEBALL_OBR_V1,
  BASEBALL_NFHS_V1,
  BASEBALL_YOUTH_V1,
  FASTPITCH_NFHS_V1,
...
export function listDiamondRulesProfiles(): readonly DiamondRulesProfile[] {
  return DIAMOND_RULES_PROFILES;
}

export function getDiamondRulesProfile(id: string, version: number): DiamondRulesProfile | null {
  return DIAMOND_RULES_PROFILES.find((profile) => profile.id === id && profile.version === version) ?? null;
}

export function requireDiamondRulesProfile(id: string, version: number): DiamondRulesProfile {
  const profile = getDiamondRulesProfile(id, version);
  if (!profile) {
    throw new DiamondDomainError('unsupported-rules-profile', `Unsupported Diamond rules profile ${id}@${String(version)}.`);
  }
  return profile;
}
