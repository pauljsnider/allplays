...
function requireId(value: unknown, label: string): string {
  ...
  if (
    ...
    throw new DiamondDomainError(
      'invalid-id',
      `${label} must be canonical, nonempty, slash- and control-free, and at most 128 characters.`
    );
  }
  return normalized;
}
...
function isDiamondIntentionalWalkLiveAnchor(
  result: DiamondCommandPayloadMap['record_pitch']['result'] | null | undefined
): boolean {
...
function validateBatterCauseResult(
  result: DiamondCommandPayloadMap['record_plate_appearance']['result'],
  destination: DiamondDestination,
  cause: DiamondRunnerAdvanceCause
) {
  if (cause === 'other') return; // Explicit unknown evidence remains partial where required.
  ...
  const compatible =
    (cause === 'batted_ball' && contactResults.includes(result)) ||
...
function validateAdvanceShape(value: unknown, options: Readonly<{ standalone?: boolean }> = {}) {
  ...
  if (options.standalone && advance.rbi === true) {
    throw new DiamondDomainError('invalid-rbi', 'A standalone runner advance has no batter to receive RBI credit.');
  }
...
function plateAppearanceOutRunnerIds(payload: DiamondCommandPayloadMap['record_plate_appearance']) {
  return [
...
function resolveDiamondPutoutCredits(fieldings: readonly DiamondFieldingChain[], actualOutRunnerIds: readonly string[]) {
  const actualOuts = new Set(actualOutRunnerIds);
  const exactPutouts = new Map<string, string>();
  fieldings.forEach((fielding) => {
    fielding.putouts?.forEach(({ runnerId, putoutBy }) => {
      if (!actualOuts.has(runnerId)) {
        throw new DiamondDomainError(
          'fielding-outs-mismatch',
          `Putout fielding evidence cites ${runnerId}, who was not retired on this play.`
        );
      }
      ...
      if (existing && existing !== putoutBy) {
        throw new DiamondDomainError(
          'fielding-outs-mismatch',
          `Putout fielding evidence assigns two fielders to the out recorded for ${runnerId}.`
        );
      }
  ...
  legacyPutoutPlayers.forEach((playerId) => {
    if (!credits.has(playerId)) credits.set(playerId, 1);
  });
  ...
  if (creditedOuts > actualOuts.size) {
    throw new DiamondDomainError(
      'fielding-outs-mismatch',
      `Putout fielding evidence credits ${String(creditedOuts)} putouts for ${String(actualOuts.size)} actual outs.`
    );
  }
...
export function validateDiamondMergedFieldingOutCredit(
...
export function validateDiamondPitchCauseEvidence(
  ...
  if (
    pitchContext?.lastPitchResult === 'illegal_pitch' &&
    ...
    throw new DiamondDomainError('pitch-cause-fielding-mismatch', 'An illegal pitch cannot also receive wild-pitch or passed-ball credit.');
  }
  if (
    result === 'interference' &&
  ...
  if (
    ...
    throw new DiamondDomainError(
      ...
      'An intentional walk without a delivered live pitch cannot carry fielding or batted-ball credits.'
    );
  }
  ...
  if (
    ...
    throw new DiamondDomainError('fielding-result-mismatch', 'A contact or dead-ball award outcome cannot also carry passed-ball credit.');
  }
  if (
...
function hasCompletePutoutEvidence(fieldings: readonly DiamondFieldingChain[], actualOutRunnerIds: readonly string[]) {
  const creditedOuts = Array.from(deriveDiamondPutoutCredits(fieldings, actualOutRunnerIds).values()).reduce(
  ...
  eventStates.forEach(({ event, before }) => {
    ...
    if (event.type === 'advance_runner') {
      ...
      if (initialState.captureMode === 'full' && payload.to === 'out') {
        if (!hasCompletePutoutEvidence(fieldingChains, outRunnerIds)) coverage = { ...coverage, fielding: 'partial' };
      }
...
function validateFieldingIds(value: unknown) {
  ...
  if (fielding.putouts !== undefined && !Array.isArray(fielding.putouts)) {
    throw new DiamondDomainError('invalid-fielding-chain', 'Per-out putout evidence must be an array.');
  }
  const putouts = fielding.putouts ?? [];
  if (putouts.length > 3) throw new DiamondDomainError('invalid-fielding-chain', 'At most three per-out putouts may be recorded.');
  ...
  if (new Set(putoutRunnerIds.map(({ runnerId }) => runnerId)).size !== putoutRunnerIds.length) {
    throw new DiamondDomainError(
      ...
      'A fielding chain may identify only one putout fielder for each retired runner.'
    );
  }
...
function validateOpposingLineupIdentities(lineups: DiamondGameState['lineups']) {
  ...
  if ([...homePlayerIds].some((playerId) => awayPlayerIds.has(playerId))) {
    throw new DiamondDomainError('opposing-lineup-player', 'Home and away lineups must use disjoint player identities.');
  }
}
...
function reduceSubstitution(
  ...
  if (dpFlex) {
    ...
    if (touchesPair) {
      if (slot.slot !== dpFlex.dpBattingSlot || !pair.has(outgoingPlayerId) || outgoingPlayerId === incomingPlayerId) {
        throw new DiamondDomainError('unsupported-dp-flex-substitution', 'DP/FLEX personnel may substitute only in their linked position.');
      }
  ...
  if (!profile.freeSubstitution && courtesyThisHalf) {
    throw new DiamondDomainError('courtesy-runner-half-inning', 'A courtesy runner cannot substitute during the same half-inning.');
  }
  if (!reentry && returningStarter && !profile.freeSubstitution) {
    throw new DiamondDomainError('reentry-required', 'A returning starter must use the re-entry command.');
  }
  if (
    ...
    throw new DiamondDomainError('invalid-reentry', 'A previously used player cannot enter as a new substitute.');
  }
  if (reentry) {
...
export function validateDiamondState(state: DiamondGameState): DiamondGameState {
  ...
  SIDES.forEach((side) => {
    ...
    if (
      histories.some((entry) => !Array.isArray(entry.substitutions) || entry.substitutions.length > 128) ||
      histories.reduce((count, entry) => count + entry.substitutions.length, 0) > 128
    ) {
      throw new DiamondDomainError('substitution-history-limit', 'A side may retain at most 128 substitution history entries.');
    }
    ...
    if (courtesyRunnerIds.some((playerId, index) => playerId !== lineup.courtesyRunnerIds[index])) {
      throw new DiamondDomainError('invalid-lineup', `${side} courtesy-runner identities must use canonical player IDs.`);
    }
    if (new Set(courtesyRunnerIds).size !== courtesyRunnerIds.length) {
      throw new DiamondDomainError('invalid-lineup', `${side} courtesy-runner identities must be unique.`);
    }
    if (courtesyRunnerIds.length > DIAMOND_MAX_COURTESY_RUNNER_IDENTITIES_PER_SIDE) {
      throw new DiamondDomainError(
    ...
    if (lineup.courtesyRunnerHalfInning) {
      ...
      if (
        history.half !== (side === 'away' ? 'top' : 'bottom') ||
        !Array.isArray(history.playerIds) ||
        history.playerIds.length > DIAMOND_MAX_COURTESY_RUNNER_IDENTITIES_PER_SIDE ||
        ...
        throw new DiamondDomainError(
          'invalid-lineup',
          'Courtesy half-inning history must contain unique known identities for the batting side.'
        );
      }
    }
    ...
    if (!dpFlex) {
      if (lineup.flexDefense) {
        throw new DiamondDomainError('invalid-dp-flex', 'FLEX history requires an established pairing.');
      }
    ...
    if (!dpSlot || ![dpSlot.starterPlayerId, dpSlot.activePlayerId, ...dpSlot.substitutions].includes(dpPlayerId)) {
      throw new DiamondDomainError('invalid-dp-flex', 'The configured DP must belong to the linked batting slot history.');
    }
    if (lineup.flexDefense) {
      const flex = lineup.flexDefense;
      if (flex.slot !== dpFlex.dpBattingSlot || flex.activePlayerId !== flexPlayerId) {
        throw new DiamondDomainError('invalid-dp-flex', 'FLEX history must match the linked active FLEX player.');
      }
  ...
  if (state.pendingBalkAwards !== undefined) {
    ...
    if (
      ...
      new Set(awards.map((award) => award.from)).size !== awards.length ||
      awards.some(
        (award: NonNullable<DiamondGameState['pendingBalkAwards']>[number]) =>
          !BASES.includes(award.from) ||
          state.bases[award.from]?.runnerId !== award.runnerId ||
          award.to !== (award.from === 'first' ? 'second' : award.from === 'second' ? 'third' : 'home')
      )
    ) {
      throw new DiamondDomainError('invalid-balk-award', 'Pending balk awards must bind each original runner to its next base.');
    }
  }
...
export function reduceDiamondEvent(state: DiamondGameState, action: DiamondReducerAction): DiamondGameState {
  ...
  if (
    ...
    throw new DiamondDomainError(pendingAwardCode, 'Resolve every mandatory pitch-infraction runner award before continuing play.');
  }
  ...
  switch (action.type) {
    ...
    case 'set_defensive_alignment': {
      ...
      if (state.lifecycle === 'active') {
        ...
        const eligibleDpRotation = Boolean(
          profile.dpFlex.enabled &&
          pair &&
    ...
    case 'record_pitch': {
      ...
      if (
        ...
        next = {
          ...next,
          pendingBalkAwards: BASES.flatMap((from) =>
            state.bases[from]
              ? [
                  {
    ...
    case 'record_plate_appearance': {
      ...
      if (
        ...
        throw new DiamondDomainError('excess-walkoff-runs', 'A non-home-run walkoff may count only the runs needed to win.');
      }
    ...
    case 'advance_runner': {
      ...
      if (pendingAwards.length)
        next = {
          ...next,
          [pendingAwardCause === 'balk' ? 'pendingBalkAwards' : 'pendingIllegalPitchAwards']: pendingAwards.filter(
            (award) => award.runnerId !== runnerId
          )
        };
    ...
    case 'add_courtesy_runner': {
      ...
      if (knownPlayerIds(state.lineups[oppositeSide(side)]).has(runnerId)) {
        throw new DiamondDomainError(
          ...
          'A courtesy runner cannot use a player identity assigned to the opposing lineup.'
        );
      }
      const placement = state.bases[base];
      ...
      if (!courtesyRunnerIds.includes(runnerId) && courtesyRunnerIds.length >= DIAMOND_MAX_COURTESY_RUNNER_IDENTITIES_PER_SIDE) {
        throw new DiamondDomainError(
          'courtesy-runner-identity-limit',
          `A side cannot retain more than ${String(DIAMOND_MAX_COURTESY_RUNNER_IDENTITIES_PER_SIDE)} courtesy-runner identities.`
        );
  ...
  if (
    state.inning.lastPitchResult !== null &&
    state.lifecycle === 'active' &&
    (action.type === 'substitute' || action.type === 're_enter' || action.type === 'set_defensive_alignment')
  ) {
    const fieldingSide = oppositeSide(getBattingSide(state));
    if (state.lineups[fieldingSide].defense.C !== next.lineups[fieldingSide].defense.C) {
      throw new DiamondDomainError(
        'pitch-catcher-change',
        'Finish the current plate appearance before changing the catcher who received its pitch.'
      );
    }
  }
