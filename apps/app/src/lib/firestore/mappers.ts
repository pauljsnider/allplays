...
import { createReplayTimestampValue, getReplayArchiveState, replayArchiveFieldNames } from '../youtubeReplay';
...
function decodeReplayTimestamp(value: unknown): unknown {
  ...
  return createReplayTimestampValue(Math.floor(milliseconds / 1000), nanoseconds) || { timestampValue: value };
}
...
function decodeReplayArchiveState(fields: Record<string, FirestoreValue> = {}) {
  return getReplayArchiveState(
    replayArchiveFieldNames.reduce<Record<string, unknown>>((result, field) => {
...
function asUniqueStringArray(value: unknown): string[] {
  ...
  return Array.from(new Set(value.map((entry) => asTrimmedString(entry)).filter((entry): entry is string => Boolean(entry))));
}
...
export function mapChatConversationRecord(value: unknown, fallbackId = ''): ChatConversationFirestoreRecord | null {
  ...
  return {
    ...
    directAccess: source.directAccess === 'accepted_friend' || source.directAccess === 'team_admin' ? source.directAccess : null,
...
export function mapScheduleEventRecord(value: unknown, fallbackId = ''): ScheduleEventFirestoreRecord | null {
  ...
  const hasReplayShareMarker = [
    ...
    (field) =>
      Object.prototype.hasOwnProperty.call(decoded, field) &&
      decoded[field] !== null &&
  ...
  return {
    ...
    sharedScheduleId: asTrimmedString(decoded.sharedScheduleId),
...
export function mapScheduleEventDocuments(documents: FirestoreDocument[] | null | undefined): ScheduleEventFirestoreRecord[] {
  return Array.isArray(documents)
    ? documents
        .map((document) => mapScheduleEventDocument(document))
        .filter((document): document is ScheduleEventFirestoreRecord => Boolean(document))
    : [];
}
