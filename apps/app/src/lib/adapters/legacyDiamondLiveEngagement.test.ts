...
describe('legacyDiamondLiveEngagement reaction subscriptions', () => {
  ...
  it('baselines cache hydration and emits later added reactions once in order', () => {
    ...
    expect(subscribeDiamondLiveReactions('team-1', 'game-1', '00000000-0000-4000-8000-000000000001', callback, onError)).toBe(unsubscribe);
    ...
    onNext(reactionSnapshot([reactionChange('added', 'new-1', 'clap')], { fromCache: true }));
  ...
  it('suppresses a server bootstrap that expands an empty cached snapshot', () => {
    ...
    subscribeDiamondLiveReactions('team-1', 'game-1', '00000000-0000-4000-8000-000000000001', callback);
    onNext(reactionSnapshot([], { fromCache: true }));
    onNext(
      reactionSnapshot([reactionChange('added', 'existing-1', 'heart'), reactionChange('added', 'existing-2', 'fire')], {
        fromCache: false
      })
    );
    expect(callback).not.toHaveBeenCalled();
  ...
  it('starts a new baseline after unsubscribe and resubscribe', () => {
    ...
    snapshots[0](reactionSnapshot([reactionChange('added', 'first-live', 'heart')], {}));
    ...
    snapshots[1](
      reactionSnapshot([reactionChange('added', 'first-live', 'heart'), reactionChange('added', 'while-disconnected', 'fire')], {})
    );
    expect(secondCallback).not.toHaveBeenCalled();
    snapshots[1](reactionSnapshot([reactionChange('added', 'second-live', 'wow')], {}));
...
function reactionSnapshot(
  ...
  return {
    docs: currentChanges.filter((change) => change.type !== 'removed').map((change) => change.doc),
    metadata,
    docChanges: () => changes
  };
}
