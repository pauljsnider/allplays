...
function liveGenerationQuery(teamId: string, gameId: string, collectionId: 'chat' | 'reactions', instanceId: string, maximum: number) {
  return query(
...
export function subscribeDiamondLiveChat<T>(
  ...
  return onSnapshot(
    ...
    (snapshot: any) => callback(snapshot.docs.map((entry: any) => ({ id: entry.id, ...entry.data() }) as T)),
    onError
  );
}
