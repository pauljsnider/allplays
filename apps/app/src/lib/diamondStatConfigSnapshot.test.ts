...
import { aggregateCoverageAwareSeasonStats, getPublicDiamondStatCatalog } from './adapters/legacyDiamondStatPresentation';
  ...
  it('allows a label edit but pins formula semantics and derived season coverage to the fixed catalog', () => {
    ...
    const mutatedConfig = {
      ...
      statDefinitions: activatedConfig.statDefinitions.map((definition) =>
        definition.id === 'avg' ? { ...definition, label: 'Batting average', formula: 'RBI/AB' } : definition
      )
    };
    const game = {
      trackingEngine: 'diamond-v2',
    ...
    const averageDefinition = getPublicDiamondStatCatalog(presentationConfig, 'player').find((definition) => definition.id === 'avg');
