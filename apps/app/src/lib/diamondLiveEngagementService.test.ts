...
describe('diamondLiveEngagementService', () => {
  ...
  it('uses the exact web callable contract and validates the bound response', async () => {
    const chatCallable = vi.fn().mockResolvedValue({
      data: {
        outcome: 'accepted',
    ...
    const reactionCallable = vi.fn().mockResolvedValue({
      data: {
        outcome: 'accepted',
        requestId: identity.requestId,
        instanceId: identity.instanceId,
    ...
    adapterMocks.httpsCallable.mockImplementation((_functions, name) => (name === 'postDiamondLiveChat' ? chatCallable : reactionCallable));
  ...
  it('loads the server-authoritative interaction window with exact web and native contracts', async () => {
    ...
    expect(adapterMocks.httpsCallable).toHaveBeenCalledWith(adapterMocks.functions, 'getPublicDiamondGame');
  ...
  it('uses the native callable bridge and rejects an unbound response', async () => {
    ...
    await expect(postDiamondLiveChat(identity, 'Great play')).rejects.toThrow('did not confirm');
    expect(nativeMocks.callNativeFirebaseFunction).toHaveBeenCalledWith(
      'postDiamondLiveChat',
  ...
  it('uses the exact manager moderation contract and requires a bound removal receipt', async () => {
    ...
    expect(adapterMocks.httpsCallable).toHaveBeenCalledWith(adapterMocks.functions, 'moderateDiamondLiveChat');
  ...
  it('requires secure randomness and never falls back to Math.random', () => {
    ...
    expect(() => createDiamondLiveEngagementRequestId(null)).toThrow('Secure live-interaction identity is unavailable');
  });
});
