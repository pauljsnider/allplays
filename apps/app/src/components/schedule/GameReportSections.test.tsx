...
describe('GameReportSections', () => {
  ...
  it('keeps roster players linkable without exposing synthetic recorded-player IDs in profile links', async () => {
    ...
    expect(screen.getByRole('link', { name: /Avery Smith/i })).toHaveAttribute('href', expect.stringContaining('playerId=player-1'));
  ...
  it('labels an incomplete manager stat scope as a retryable whole-public fallback', async () => {
    ...
    expect(
      screen.getByText('Internal projection unavailable; showing the last complete public projection. Refresh to retry.')
    ).toBeTruthy();
