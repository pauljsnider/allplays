...
export type AttentionPanelSectionId = 'availability' | 'rideshare' | 'assignments' | 'game';

export type AttentionPanelItem = {
  title: string;
  detail: string;
  section: AttentionPanelSectionId;
};

export function AttentionPanel({ items, onSelectSection }: { items: AttentionPanelItem[]; onSelectSection: (sectionId: AttentionPanelSectionId) => void }) {
  if (!items.length) {
    return (
