...
export function DocumentAuthLink({ route, className, children }: { route: string; className?: string; children: ReactNode }) {
  ...
  return (
    <a href={documentRoute} className={className}>
      {children}
    </a>
  );
}
    ...
    <div className="min-h-screen bg-gray-50 px-4 py-6">
      <div className="mx-auto flex min-h-[calc(100vh-48px)] w-full max-w-md flex-col justify-center">
        <Link to={safeBrandTo} className="mb-5 flex items-center gap-3">
          ...
          <span>
            <span className="block text-lg leading-tight font-black text-gray-950">ALL PLAYS</span>
            <span className="text-primary-700 block text-xs font-extrabold tracking-[0.04em] uppercase">{eyebrow}</span>
          </span>
        </Link>
        {documentBackTo ? (
          <DocumentAuthLink route={documentBackTo} className="ghost-button mb-3 !min-h-9 w-fit !px-3 !py-1.5 text-sm">
            <ChevronLeft className="h-4 w-4" aria-hidden="true" />
            {backLabel}
          </DocumentAuthLink>
        ) : backTo ? (
          <Link to={backTo} className="ghost-button mb-3 !min-h-9 w-fit !px-3 !py-1.5 text-sm">
            <ChevronLeft className="h-4 w-4" aria-hidden="true" />
            {backLabel}
          </Link>
        ) : null}
