 ...
 * redemption already wrote. This script finds access codes whose usedBy uid
 * has no Firebase Auth record and reports them. With --apply --code CODE it:
 *   - un-marks the code,
 *   - deletes the orphaned private/public user docs, and
 *   - removes the orphaned uid from the player's private parents list.
 *
 * DRY RUN by default. Apply mode requires one explicit --code value.
 *
 * Usage:
 *   node _migration/fix-orphaned-invite-redemptions.js
 *   node _migration/fix-orphaned-invite-redemptions.js --code 7PPHXY3R
...
const onlyCode = codeFlagIdx !== -1
    ? String(process.argv[codeFlagIdx + 1] || '').trim().toUpperCase()
    : null;

if (APPLY && !onlyCode) {
    throw new Error('Apply mode requires an explicit --code value; bulk writes are disabled.');
}

function getAdminApp() {
    if (!getApps().length) {
        const serviceAccount = JSON.parse(readFileSync(new URL('./serviceAccountKey.json', import.meta.url), 'utf8'));
        initializeApp({ credential: cert(serviceAccount), projectId: 'game-flow-c6311' });
    }
    return getApps()[0];
}
...
async function repairCode(db, codeSnap, { apply }) {
    ...
    if (!apply) {
        console.log('    DRY RUN: would un-mark code and delete orphaned user docs');
        return;
    }
...
async function main() {
    ...
    for (const codeSnap of snapshot.docs) {
        const codeData = codeSnap.data() || {};
        const uid = String(codeData.usedBy || '').trim();
        if (!uid || await authRecordExists(auth, uid)) {
            continue;
        }
        orphaned += 1;
    ...
    console.log(`Done. ${orphaned} orphaned redemption(s) ${APPLY ? 'repaired' : 'found (dry run — pass --apply --code CODE to fix)'}.`);
}
