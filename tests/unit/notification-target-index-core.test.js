...
describe('notification target index core helpers', () => {
    ...
    it('uses team recipient indexes and falls back for users missing from partial indexes', () => {
        ...
        expect(targetResolverSource).toContain('const missingUsers = resolutionUsers.filter((user) => !indexedUserIds.has(user.uid));');
        expect(targetResolverSource).toContain('getLegacyTargetsForCategory(teamId, category, missingUsers, actorUid, audienceContext)');
        expect(targetResolverSource).toContain("await backfillNotificationRecipientsForTeam(teamId, missingUsers, { skipLegacyCleanup: true });");
