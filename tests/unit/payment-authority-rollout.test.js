...
describe('Stripe payment-authority rollout gate', () => {
    ...
    it('documents a fail-closed frozen cutover and reopens only after the post-deploy assertion', () => {
        ...
        expect(runbook).toContain('the maintenance freeze is not active yet');
        expect(runbook).toContain('The verified maintenance window starts only after all three gates are simultaneously proven');
        expect(runbook).toContain('the control document cannot make those old revisions safe');
        const inventoryIndex = runbook.indexOf('1. Before claiming or writing any freeze');
        const transportIndex = runbook.indexOf('2. Announce maintenance, then establish transport quiescence');
        const controlIndex = runbook.indexOf('3. Only after every deployed legacy mutation callable is transport-blocked');
        const bootstrapIndex = runbook.indexOf('4. Bootstrap only the two reviewed rollout callables');
