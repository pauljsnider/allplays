...
describe('scoped fallback uploads', () => {
    ...
    it('uploads chat attachments directly to the primary scoped path without image-project auth', async () => {
        const { uploadChatImage } = await import('../../js/db.js?v=4433172-scoped-fallback-uploads');
    ...
    it('deletes new scoped chat media from primary storage and legacy chat media from image storage', async () => {
        const { deleteUploadedChatAttachments } = await import('../../js/db.js?v=4433172-scoped-fallback-uploads');
    ...
    it('uses the primary scoped path for browser player photos and deletes it on rollback', async () => {
        const { deleteLegacyImageUpload, uploadPlayerPhoto } = await import('../../js/db.js?v=4433172-scoped-fallback-uploads');
    ...
    it('deletes a completed browser team photo when its download URL lookup fails', async () => {
        firebaseMocks.getDownloadURL.mockRejectedValueOnce(new Error('url lookup failed'));
        const { uploadTeamPhoto } = await import('../../js/db.js?v=4433172-scoped-fallback-uploads');
    ...
    it.each([
        ...
        const db = await import('../../js/db.js?v=4433172-rejected-profile-photo-upload');
    ...
    it.each([
        ...
        const db = await import('../../js/db.js?v=4433172-rejected-media-upload');
    ...
    it('does not let a failed same-millisecond upload delete a concurrent successful candidate', async () => {
        ...
        const { uploadTeamPhoto } = await import('../../js/db.js?v=4433172-concurrent-profile-photo-upload');
    ...
    it('uses the signed-in primary Storage identity for browser own-profile photos', async () => {
        const { uploadUserPhoto } = await import('../../js/db.js?v=4433172-scoped-fallback-uploads');
    ...
    it('uploads stat sheets exclusively to the primary game-scoped path', async () => {
        const { deleteUploadedMediaObjects, uploadStatSheetPhoto } = await import('../../js/db.js?v=4433172-scoped-fallback-uploads');
    ...
    it('requires nonempty string team and game ids before starting a stat sheet upload', async () => {
        const { uploadStatSheetPhoto } = await import('../../js/db.js?v=4433172-scoped-fallback-uploads');
        const file = {
            name: 'box score.png',
            size: 123,
            type: 'image/png'
    ...
    it('falls back to a team-scoped drill path and deletes it from primary storage on rollback', async () => {
        const { deleteUploadedMediaObjects, uploadDrillDiagram } = await import('../../js/db.js?v=4433172-scoped-fallback-uploads');
    ...
    it('falls back to primary drill storage when image-project authentication fails', async () => {
        ...
        const { uploadDrillDiagram } = await import('../../js/db.js?v=4433172-scoped-fallback-uploads');
    ...
    it('deletes a game clip from image storage when its game update fails', async () => {
        ...
        const { deleteUploadedMediaObjects, uploadGameClip } = await import('../../js/db.js?v=4433172-scoped-fallback-uploads');
    ...
    it('falls back to primary game-clip storage when image-project authentication fails', async () => {
        imageAuthMocks.requireImageAuth.mockRejectedValueOnce(new Error('image auth unavailable'));
        const { uploadGameClip } = await import('../../js/db.js?v=4433172-scoped-fallback-uploads');
