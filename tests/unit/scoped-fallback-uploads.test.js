...
describe('scoped fallback uploads', () => {
    ...
    it('uploads chat attachments directly to the primary scoped path without image-project auth', async () => {
        const { uploadChatImage } = await import('../../js/db.js?v=153-scoped-fallback-uploads');
    ...
    it('deletes new scoped chat media from primary storage and legacy chat media from image storage', async () => {
        const { deleteUploadedChatAttachments } = await import('../../js/db.js?v=153-scoped-fallback-uploads');
    ...
    it.each([
        ...
        const db = await import('../../js/db.js?v=153-rejected-profile-photo-upload');
    ...
    it.each([
        ...
        const db = await import('../../js/db.js?v=153-rejected-media-upload');
    ...
    it('does not let a failed same-millisecond upload delete a concurrent successful candidate', async () => {
        ...
        const { uploadTeamPhoto } = await import('../../js/db.js?v=153-concurrent-profile-photo-upload');
    ...
    it('falls back to a team-scoped stat sheet path when image storage rejects the upload', async () => {
        const { deleteUploadedMediaObjects, uploadStatSheetPhoto } = await import('../../js/db.js?v=153-scoped-fallback-uploads');
    ...
    it('falls back to primary stat sheet storage when image-project authentication fails', async () => {
        ...
        const { uploadStatSheetPhoto } = await import('../../js/db.js?v=153-scoped-fallback-uploads');
    ...
    it('falls back to a team-scoped drill path and deletes it from primary storage on rollback', async () => {
        const { deleteUploadedMediaObjects, uploadDrillDiagram } = await import('../../js/db.js?v=153-scoped-fallback-uploads');
    ...
    it('falls back to primary drill storage when image-project authentication fails', async () => {
        imageAuthMocks.requireImageAuth.mockRejectedValueOnce(new Error('image auth unavailable'));
        const { uploadDrillDiagram } = await import('../../js/db.js?v=153-scoped-fallback-uploads');
    ...
    it('deletes a game clip from image storage when its game update fails', async () => {
        ...
        const { deleteUploadedMediaObjects, uploadGameClip } = await import('../../js/db.js?v=153-scoped-fallback-uploads');
    ...
    it('falls back to primary game-clip storage when image-project authentication fails', async () => {
        imageAuthMocks.requireImageAuth.mockRejectedValueOnce(new Error('image auth unavailable'));
        const { uploadGameClip } = await import('../../js/db.js?v=153-scoped-fallback-uploads');
