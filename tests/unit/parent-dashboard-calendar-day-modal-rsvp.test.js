...
` + match[1]
        ...
        .replace("import { createParentDashboardRsvpController } from './js/parent-dashboard-rsvp-controls.js?v=2';", 'const { createParentDashboardRsvpController } = deps.parentDashboardRsvpControls;')
        .replace("import { getEventRideshareSummary, getOfferSeatInfo } from './js/rideshare-helpers.js?v=2';", 'const { getEventRideshareSummary, getOfferSeatInfo } = deps.rideshareHelpers;')
        ...
        .replace("import { resolveSelectedRideChildId, getRideOfferUiState, createRideRequestHandlers } from './js/parent-dashboard-rideshare-controls.js?v=2';", 'const { resolveSelectedRideChildId, getRideOfferUiState, createRideRequestHandlers } = deps.parentDashboardRideshareControls;')
...
function createDeps(submitRecorder) {
    return {
        db: {
            ...
            async submitRsvpForPlayer(teamId, gameId, currentUserId, payload) {
                submitRecorder.playerCalls.push({ teamId, gameId, currentUserId, payload });
                return submitRecorder.updatedSummary;
            },
...
describe('parent dashboard calendar day modal RSVP flow', () => {
    it('renders separate shared-game child RSVP controls and submits only the chosen child', async () => {
        vi.useFakeTimers();
        ...
        try {
            ...
            expect(beforeHtml).toContain('data-child-id="child-a"');
            expect(beforeHtml).toContain('data-child-id="child-b"');
            expect(beforeHtml).not.toContain('data-child-ids="child-a,child-b"');
            expect(beforeHtml).toContain('Avery');
            expect(beforeHtml).toContain('Blake');
            ...
            expect(submitRecorder.playerCalls).toEqual([
                {
                    teamId: 'team-1',
                    gameId: 'game-1',
            ...
            expect(afterHtml).toContain('data-child-id="child-b"');
