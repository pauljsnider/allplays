...
describe('player tracking summary', () => {
    ...
    it('wires team page and Firestore rules for family read-only access', () => {
        ...
        expect(dbSource).toContain('getDocs(query(');
        expect(dbSource).toContain("where('visibility', '==', 'public')");
        expect(dbSource).toContain("where('status', '==', 'active')");
        expect(dbSource).toContain("where('archived', '==', false)");
        expect(dbSource).toContain("where('active', '==', true)");
        ...
        expect(rules).toContain('canReadNestedTrackingStatus(teamId, itemId, trackingId, resource.data)');
