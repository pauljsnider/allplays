...
describe('admin invite signup cache busting', () => {
    it('pins fresh module versions for the admin invite signup path', () => {
        ...
        expect(authSource).toContain("from './db.js?v=112';");
    ...
    it('pins fresh invite acceptance module versions for admin invite redemption', () => {
        ...
        expect(acceptInviteSource).toContain(
            "import { validateAccessCode, redeemParentInvite, redeemHouseholdInvite, redeemCoParentInvite, redeemFriendInvite, updateUserProfile, updateTeam, getTeam, getUserProfile, markAccessCodeAsUsed } from './js/db.js?v=112';"
        );
        expect(acceptInviteSource).toContain(
    ...
    it('bumps auth module consumers after signup flow changes', () => {
        const authConsumers = {
            ...
            'js/track-basketball.js': 'auth.js?v=124'
        };
        ...
        expect(editTeamSource).toContain("import { checkAuth, sendInviteEmail } from './js/auth.js?v=124';");
    ...
    it('bumps the shared header logout import with auth.js consumers', () => {
        ...
        const logoutImportMatches = utilsSource.match(/const \{ logout \} = await import\('\.\/auth\.js\?v=124'\);/g) || [];
    ...
    it('pins every deployed auth and db consumer to the App Check-aware module keys', () => {
        ...
        const staleConsumers = deployedSources.flatMap((relativePath) => {
            const source = readFileSync(resolve(process.cwd(), relativePath), 'utf8');
            const staleImports = source.match(/(?:(?<![\w-])auth\.js\?v=(?!124\b)\d+|(?<![\w-])db\.js\?v=(?!112\b)\d+|(?<![\w-])utils\.js\?v=(?!18\b)\d+)\b/g) || [];
