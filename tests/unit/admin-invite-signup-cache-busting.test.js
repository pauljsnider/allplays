...
describe('admin invite signup cache busting', () => {
    it('pins fresh module versions for the admin invite signup path', () => {
        ...
        expect(authSource).toContain("from './db.js?v=63';");
    });
    ...
    it('pins fresh invite acceptance module versions for admin invite redemption', () => {
        ...
        expect(acceptInviteSource).toContain(
            "import { validateAccessCode, redeemParentInvite, redeemHouseholdInvite, redeemAdminInviteAtomically, updateUserProfile, updateTeam, getTeam, getUserProfile, markAccessCodeAsUsed } from './js/db.js?v=63';"
        );
        expect(acceptInviteSource).toContain(
    ...
    it('bumps auth module consumers after signup flow changes', () => {
        const authConsumers = {
            'login.html': 'auth.js?v=32',
            'accept-invite.html': 'auth.js?v=32',
            'edit-team.html': 'auth.js?v=32',
            'js/admin.js': 'auth.js?v=32',
            'js/live-game.js': 'auth.js?v=32',
            'js/live-tracker.js': 'auth.js?v=32',
            'js/track-basketball.js': 'auth.js?v=32'
        };
    ...
    it('keeps the shared header logout import pinned to auth.js v22', () => {
        ...
        expect(utilsSource).not.toContain("const { logout } = await import('./auth.js?v=32');");
        expect(utilsSource).not.toContain("const { logout } = await import('./auth.js?v=32');");
    });
});
