...
const videoSource = readFileSync(new URL('../../js/live-game-video.js', import.meta.url), 'utf8');
const chatSource = readFileSync(new URL('../../js/live-game-chat.js', import.meta.url), 'utf8');
...
describe('live game overlay page', () => {
    it('ships as a separate no-index broadcast canvas with accessible overlay regions', () => {
        ...
        expect(html).toContain('js/live-game-overlay.js?v=45');
    });
    ...
    it('keeps the local demo isolated while wiring canonical subscriptions and authenticated chat posting', () => {
        ...
        expect(source).toContain("return import('./db.js?v=4433196')");
        expect(source).toContain("import('./live-game-state.js?v=44')");
        expect(source).toContain('stateTools.applyResetEventState');
        expect(source).toContain('reconcileOverlayLiveEvents');
        expect(source).toContain("from './live-game-overlay-model.js?v=29'");
        expect(source).toContain("import('./live-game-video.js?v=443321')");
        ...
        expect(source).toContain("import('./auth.js?v=4433200')");
        expect(source).toContain("import('./live-game-chat.js?v=5')");
        expect(currentLiveGameSource).toContain("from './live-game-overlay-model.js?v=29'");
        expect(currentLiveGameSource).toContain("from './live-game-video.js?v=443321'");
        expect(currentLiveGameSource).toContain("from './live-game-chat.js?v=5'");
        expect(modelSource).toContain("from './game-replay-video.js?v=4'");
        expect(videoSource).toContain("from './game-replay-video.js?v=4'");
        expect(chatSource).toContain("from './game-replay-video.js?v=4'");
        ...
        expect(source).toContain("from './utils.js?v=443372'");
        expect(source).toContain("from './live-game-announcer.js?v=1'");
        expect(source).toContain("from './game-replay-video.js?v=4'");
        expect(source).toContain("import('./game-replay-service.js?v=2')");
        expect(source).toContain('gameReplayService.getPlayback({ teamId, gameId })');
        expect(source).toContain("unavailableReason === 'team-pass-required'");
        expect(source).not.toContain("import('./team-entitlements.js");
    ...
    it('provides focus, panel, keyboard, demo, and reciprocal viewer navigation', () => {
        ...
        expect(currentLiveGameSource).toContain("hasReplayPlayback ? 'Watch Replay' : 'Watch Live'");
        expect(currentLiveGameSource).toContain("els.overlayViewLink.classList.toggle('hidden', !hasReplayPlayback && !hasLivePlayback)");
    });
