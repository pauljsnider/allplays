...
describe('registration failed payment reminder wiring', () => {
    ...
    it('records auditable reminder metadata on the registration and resolves it when paid', () => {
        ...
        expect(source).toContain("effectivePaymentStatus: ['disputed', 'dispute_lost'].includes(aggregateFinancialState.financialStatus)");
        expect(source).toContain('chargeLedger.refundedAmountCents > 0');
