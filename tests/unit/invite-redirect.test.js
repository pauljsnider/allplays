...
describe('invite redirect helper', () => {
    ...
    it('routes login to invite acceptance when redemption is requested', () => {
        expect(getPostAuthRedirectUrl('dashboard.html', 'abcd1234', true)).toBe('https://allplays.ai/app/#/accept-invite?code=ABCD1234');
    });

    it('preserves the admin invite type when redemption is requested', () => {
        expect(getPostAuthRedirectUrl('dashboard.html', 'abcd1234', true, 'Admin')).toBe('https://allplays.ai/app/#/accept-invite?code=ABCD1234&type=admin');
    });

    it('preserves household invite redirects when redemption is requested', () => {
        expect(getPostAuthRedirectUrl('dashboard.html', 'abcd1234', true, 'household')).toBe('https://allplays.ai/app/#/accept-invite?code=ABCD1234&type=household');
    });

    it('normalizes household_invite redirects to the household accept-invite route', () => {
        expect(getPostAuthRedirectUrl('dashboard.html', 'abcd1234', true, 'household_invite')).toBe('https://allplays.ai/app/#/accept-invite?code=ABCD1234&type=household');
    });

    it('preserves standard and co-parent join-code redirects', () => {
        expect(getPostAuthRedirectUrl('dashboard.html', 'abcd1234', true, 'standard')).toBe('https://allplays.ai/app/#/accept-invite?code=ABCD1234&type=standard');
        expect(getPostAuthRedirectUrl('dashboard.html', 'abcd1234', true, 'coparent_invite')).toBe('https://allplays.ai/app/#/accept-invite?code=ABCD1234&type=coparent');
    });
