...
function rootUrl(path) {
    ...
    return new URL(path, appUrl.origin).toString();
}
...
for (const definition of redirectCases) {
    test(`legacy ${definition.name} redirects to the app without a loop`, async ({ page }) => {
        const visited = [];
        page.on('framenavigated', (frame) => {
            if (frame === page.mainFrame()) visited.push(new URL(frame.url()).pathname);
        });
        await page.goto(rootUrl(definition.source), { waitUntil: 'domcontentloaded' });
