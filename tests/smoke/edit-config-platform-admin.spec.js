...
async function mockDependencies(page) {
    ...
    await page.route(/\/js\/edit-config-access\.js(?:\?v=\d+)?$/, (route) => route.fulfill({ status: 200, contentType: 'application/javascript', body: EDIT_CONFIG_ACCESS_STUB }));
    ...
    await page.route('**/js/stat-leaderboards.js?v=4', (route) => route.fulfill({ status: 200, contentType: 'application/javascript', body: STAT_LEADERBOARDS_STUB }));
    await page.route('**/js/stat-config-presets.js?v=11', (route) => route.fulfill({ status: 200, contentType: 'application/javascript', body: STAT_CONFIG_PRESETS_STUB }));
}
