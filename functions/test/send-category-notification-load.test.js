...
describe('sendCategoryNotification load coverage', () => {
    it('handles a 500-recipient indexed send without legacy per-user scans', async () => {
        ...
        try {
            ...
            assert.equal(env.counts.inboxCleanupQueries, 500);
            assert.equal(env.counts.inboxCleanupLimitQueries, 500);
            assert.deepEqual(new Set(env.inboxCleanupLimits), new Set([51]));
