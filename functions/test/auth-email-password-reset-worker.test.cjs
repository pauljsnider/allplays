...
function createHarness(overrides = {}) {
  const calls = { queued: [], released: [], deleted: 0, errors: [] };
  const auth = {
  ...
  const worker = createPasswordResetEmailWorker({
    ...
    async releaseDelivery(...args) { calls.released.push(args); },
    isAlreadyExistsError: (error) => error?.code === 'already-exists',
    ...overrides,
    auth
  });
  const process = (data = { type: 'password_reset', email: 'Coach@Example.com' }, requestId = 'request-1') =>
...
test('worker silently releases missing accounts after the callable has returned', async () => {
  const { calls, process } = createHarness({
    auth: {
  ...
  await process();
  assert.deepEqual(calls.queued, []);
  assert.deepEqual(calls.released, [['password_reset', 'coach@example.com']]);
  assert.deepEqual(calls.errors, []);
  assert.equal(calls.deleted, 1);
});

test('worker releases backend failures and always removes the internal request', async () => {
  const { calls, process } = createHarness({
    auth: {
...
test('worker rejects malformed internal requests without sending mail', async () => {
  const { calls, process } = createHarness();
  await process({ type: 'verification', email: 'coach@example.com' });
  assert.deepEqual(calls.queued, []);
  assert.deepEqual(calls.released, [['password_reset', 'coach@example.com']]);
  assert.equal(calls.deleted, 1);
});
