...
function privateNoteStateForStorage(state) {
    ...
    return state.currentScorerUid === null ? state : { ...state, currentScorerUid: exports.DIAMOND_PRIVATE_NOTE_ACTOR_REDACTION };
}
function resultForPrivateNoteStorage(result, event) {
    return event.actorUid !== exports.DIAMOND_PRIVATE_NOTE_ACTOR_REDACTION ? result : { ...result, state: event.after };
}
function resultForPrivateNoteResponse(result, event, currentScorerUid) {
    return event.actorUid !== exports.DIAMOND_PRIVATE_NOTE_ACTOR_REDACTION ? result : { ...result, state: { ...result.state, currentScorerUid } };
}
...
function isCanonicalPrivateNoteMaterialEvent(event) {
    ...
    return ((event.type === 'void_event' || event.type === 'supersede_event') && event.payload.reason === exports.DIAMOND_PRIVATE_NOTE_REASON_TOMBSTONE);
}
...
function isLeaseExemptPrivateNoteCommand(ledger, command, privateTargetVerified) {
    if (command.type === 'private_note')
        return true;
    if (!isCorrectionCommand(command))
        return false;
...
function observeEffectiveEventParticipants(state, event, tracker) {
    if (event.type === 'advance_runner' &&
        ['pickoff', 'balk'].includes(event.payload.cause)) {
