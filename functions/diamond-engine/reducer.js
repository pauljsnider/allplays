...
exports.isDiamondDroppedThirdStrikeAdvance = isDiamondDroppedThirdStrikeAdvance;
exports.hasCompleteDiamondDroppedThirdStrikeCause = hasCompleteDiamondDroppedThirdStrikeCause;
...
const DROPPED_THIRD_STRIKE_ADVANCE_CAUSES = ['wild_pitch', 'passed_ball', 'error'];
...
function validateScoringCredit(value, label) {
    ...
    if (value.rbi === true && (value.to !== 'home' || value.countsRun === false)) {
        throw new contracts_1.DiamondDomainError('invalid-rbi', 'Positive RBI credit requires an advance that records a counted run.');
    }
...
function isDiamondDroppedThirdStrikeAdvance(result, destination) {
...
function validatePlateAppearanceRunnerAdvance(state, result, advance) {
    ...
    if (contradictory) {
        throw new contracts_1.DiamondDomainError('runner-cause-result-mismatch', 'The runner advance contradicts the plate-appearance result or forced award.');
    }
}
...
function plateAppearanceOutRunnerIds(payload) {
    return [
        ...(payload.batterAdvance.to === 'out' ? [payload.batterId] : []),
        ...payload.runnerAdvances.flatMap((advance) => (advance.to === 'out' ? [advance.runnerId] : []))
    ];
}
function resolveDiamondPutoutCredits(fieldings, actualOutRunnerIds) {
    const actualOuts = new Set(actualOutRunnerIds);
    const exactPutouts = new Map();
    fieldings.forEach((fielding) => {
        fielding.putouts?.forEach(({ runnerId, putoutBy }) => {
            if (!actualOuts.has(runnerId)) {
                throw new contracts_1.DiamondDomainError('fielding-outs-mismatch', `Putout fielding evidence cites ${runnerId}, who was not retired on this play.`);
            }
            const existing = exactPutouts.get(runnerId);
            if (existing && existing !== putoutBy) {
                throw new contracts_1.DiamondDomainError('fielding-outs-mismatch', `Putout fielding evidence assigns two fielders to the out recorded for ${runnerId}.`);
            }
            exactPutouts.set(runnerId, putoutBy);
        });
    });
    const credits = new Map();
    exactPutouts.forEach((putoutBy) => credits.set(putoutBy, (credits.get(putoutBy) ?? 0) + 1));
    // Legacy credits have no out identity, so deduplicate them by fielder. Exact
    // credits preserve multiplicity because distinct retired runners identify distinct outs.
    const legacyPutoutPlayers = new Set(fieldings.flatMap((fielding) => (fielding.putoutBy ? [fielding.putoutBy] : [])));
    legacyPutoutPlayers.forEach((playerId) => {
        if (!credits.has(playerId))
            credits.set(playerId, 1);
    });
    const creditedOuts = Array.from(credits.values()).reduce((total, count) => total + count, 0);
    if (creditedOuts > actualOuts.size) {
        throw new contracts_1.DiamondDomainError('fielding-outs-mismatch', `Putout fielding evidence credits ${String(creditedOuts)} putouts for ${String(actualOuts.size)} actual outs.`);
    }
    const assistPlayers = new Set(fieldings.flatMap((fielding) => fielding.assists ?? []));
    if (exactPutouts.size === 0 && actualOuts.size > 1 && legacyPutoutPlayers.size === 1 && assistPlayers.size === 0) {
        credits.set(legacyPutoutPlayers.values().next().value, actualOuts.size);
    }
    return credits;
}
function validateDiamondMergedFieldingOutCredit(fieldings, actualOutRunnerIds) {
    resolveDiamondPutoutCredits(fieldings, actualOutRunnerIds);
}
...
function hasCompletePutoutEvidence(fieldings, actualOutRunnerIds) {
    ...
    return creditedOuts === new Set(actualOutRunnerIds).size;
}
...
function validateHomeRunRbiEvidence(result, scoringAdvances, runsBattedIn) {
    if (result !== 'home_run')
        return;
    if (scoringAdvances.some((advance) => advance.rbi === false)) {
        throw new contracts_1.DiamondDomainError('invalid-rbi', 'Every counted run on a home run must credit the batter with an RBI.');
    }
    if (runsBattedIn !== undefined && runsBattedIn !== scoringAdvances.length) {
        throw new contracts_1.DiamondDomainError('invalid-rbi', 'A home-run RBI total must equal the number of counted runs on the play.');
    }
}
function deriveDiamondCoverageFromEventStates(initialState, eventStates) {
    ...
    eventStates.forEach(({ event, before }) => {
        ...
        if (event.type === 'record_plate_appearance') {
            ...
            const outRunnerIds = plateAppearanceOutRunnerIds(payload);
            ...
            if (initialState.captureMode === 'full') {
                ...
                if (!hasKnownDroppedThirdStrikeCause)
                    coverage = { ...coverage, pitching: 'partial' };
                if (payload.outsOnPlay > 0 && !hasCompletePutoutEvidence(fieldingChains, outRunnerIds)) {
                    coverage = { ...coverage, fielding: 'partial' };
                }
                if (!hasKnownDroppedThirdStrikeCause ||
        ...
        if (event.type === 'advance_runner') {
            ...
            if (initialState.captureMode === 'full' && payload.to === 'out') {
                if (!hasCompletePutoutEvidence(fieldingChains, outRunnerIds))
                    coverage = { ...coverage, fielding: 'partial' };
            }
...
function validateFieldingIds(value) {
    ...
    if (fielding.putouts !== undefined && !Array.isArray(fielding.putouts)) {
        throw new contracts_1.DiamondDomainError('invalid-fielding-chain', 'Per-out putout evidence must be an array.');
    }
    const putouts = fielding.putouts ?? [];
    if (putouts.length > 3)
        throw new contracts_1.DiamondDomainError('invalid-fielding-chain', 'At most three per-out putouts may be recorded.');
    const putoutRunnerIds = putouts.map((value, index) => {
        const putout = requireRecord(value, `fielding.putouts[${String(index)}]`);
        requireOnlyFields(putout, new Set(['runnerId', 'putoutBy']), `fielding.putouts[${String(index)}]`);
        return {
            runnerId: requireId(putout.runnerId, 'putout runnerId'),
            putoutBy: requireId(putout.putoutBy, 'putout fielder')
        };
    });
    if (new Set(putoutRunnerIds.map(({ runnerId }) => runnerId)).size !== putoutRunnerIds.length) {
        throw new contracts_1.DiamondDomainError('invalid-fielding-chain', 'A fielding chain may identify only one putout fielder for each retired runner.');
    }
...
function knownPlayerIds(lineup) {
    return new Set([
        ...
        ...(lineup.flexDefense
            ? [lineup.flexDefense.activePlayerId, lineup.flexDefense.starterPlayerId, ...lineup.flexDefense.substitutions]
            : []),
...
function validateInlineFieldingParticipants(state, value) {
    ...
    const creditedIds = [
        value.putoutBy,
        ...(value.putouts ?? []).map((putout) => putout.putoutBy),
        ...(value.assists ?? []),
...
function validateOutcomeDestination(state, result, destination) {
    ...
    const advancesOnDroppedThirdStrike = (result === 'strikeout' && destination === 'first') || (result === 'dropped_third_strike' && destination !== 'out');
    if (advancesOnDroppedThirdStrike) {
        if (state.inning.lastPitchResult === 'foul_bunt' && state.inning.strikes === 3) {
            throw new contracts_1.DiamondDomainError('dropped-third-strike-ineligible', 'A third-strike foul bunt is a dead-ball out.');
        }
        const profile = (0, rules_1.requireDiamondRulesProfile)(state.rulesProfileId, state.rulesProfileVersion);
...
function retainPitchAdvanceCause(state, advances, fielding) {
    if (!state.inning.lastPitchResult || !isDiamondDeliveredPitch(state.inning.lastPitchResult))
        return state;
    const causes = [
        ...advances.filter((advance) => advance.cause === 'wild_pitch' || advance.cause === 'passed_ball'),
        ...(fielding?.passedBallBy ? [{ cause: 'passed_ball' }] : [])
    ];
    validateDiamondPitchCauseEvidence([...causes, ...(state.inning.lastPitchAdvanceCause ? [{ cause: state.inning.lastPitchAdvanceCause }] : [])], []);
    const cause = causes[0]?.cause;
    if (cause !== 'wild_pitch' && cause !== 'passed_ball')
        return state;
    ...
    moves.forEach((move) => {
        ...
        if (move.to === 'home') {
            if (!playEndsHalf && move.countsRun === false) {
                throw new contracts_1.DiamondDomainError('run-nullification-requires-third-out', 'A run may be marked not counting only on a play that records the third out.');
            }
...
function reduceSubstitution(state, payload, reentry) {
    ...
    const slot = returningRetiredDhStarter
        ...
        : defensiveOnly || replacingFlex
            ...
            : battingSlot;
    if (returningRetiredDhStarter ? battingSlot.activePlayerId !== outgoingPlayerId : !restoringFlex && slot.activePlayerId !== outgoingPlayerId) {
    ...
    const pairExchange = Boolean(!restoringFlex &&
        dpFlex &&
    ...
    if (dpFlex) {
        ...
        if (touchesPair) {
            if (slot.slot !== dpFlex.dpBattingSlot || !pair.has(outgoingPlayerId) || outgoingPlayerId === incomingPlayerId) {
                throw new contracts_1.DiamondDomainError('unsupported-dp-flex-substitution', 'DP/FLEX personnel may substitute only in their linked position.');
            }
            ...
            if ((flexOnly || pairExchange) && payload.defensivePosition && payload.defensivePosition !== dpFlex.flexDefensivePosition) {
            ...
            if (!pairExchange) {
                dpFlex = replacingFlex ? { ...dpFlex, flexPlayerId: incomingPlayerId } : { ...dpFlex, dpPlayerId: incomingPlayerId };
            }
        }
    }
    ...
    if (!profile.freeSubstitution &&
        ...
        !endingDh &&
        !dhTakingDefense &&
        [...order, ...(lineup.dhDefense ? [lineup.dhDefense] : []), ...(lineup.flexDefense ? [lineup.flexDefense] : [])].some((entry) => entry.starterPlayerId === incomingPlayerId || entry.substitutions.includes(incomingPlayerId))) {
    ...
    const replacement = {
        ...
        battingRole: endingDh || lineup.dhTerminated
            ? 'regular'
    ...
    if (!defensiveOnly)
        order[index] = returningRetiredDhStarter
            ...
            : replacingFlex
                ...
                : replacement;
    if (dhTakingDefense)
        order[index] = { ...battingSlot, battingRole: 'regular' };
...
function validateDiamondState(state) {
    ...
    if (state.inning.lastPitchAdvanceCause !== null) {
        ...
        if (!state.inning.lastPitchResult || state.inning.pitchesInPlateAppearance === 0) {
            throw new contracts_1.DiamondDomainError('invalid-pitch-cause-anchor', 'Pitch advance credit requires a delivered pitch anchor.');
        }
    }
    ...
    SIDES.forEach((side) => {
        ...
        if (lineup.dhTerminated !== undefined &&
            (lineup.dhTerminated !== true || !profile.allowsDh || !lineup.dhDefense || order.some((entry) => entry.battingRole === 'dh'))) {
            throw new contracts_1.DiamondDomainError('invalid-lineup', 'A terminated DH must retain history without an active DH role.');
        }
        if (lineup.dhDefense) {
        ...
        if (state.lifecycle !== 'configured' &&
            ...
            throw new contracts_1.DiamondDomainError('invalid-defensive-personnel', 'A traditional DH cannot defend while a separate non-batting defender remains.');
        }
        ...
        if (lineup.courtesyRunnerRoles !== undefined) {
            ...
            if (!Array.isArray(roles) ||
                roles.length > 2 * contracts_1.DIAMOND_MAX_COURTESY_RUNNER_IDENTITIES_PER_SIDE ||
                ...
                throw new contracts_1.DiamondDomainError('invalid-lineup', 'Courtesy role history must bind known identities to valid, nonconflicting roles.');
            }
        }
        ...
        if (!dpFlex) {
            if (lineup.flexDefense) {
                throw new contracts_1.DiamondDomainError('invalid-dp-flex', 'FLEX history requires an established pairing.');
            }
        ...
        if (!dpSlot || ![dpSlot.starterPlayerId, dpSlot.activePlayerId, ...dpSlot.substitutions].includes(dpPlayerId)) {
            throw new contracts_1.DiamondDomainError('invalid-dp-flex', 'The configured DP must belong to the linked batting slot history.');
        }
        if (lineup.flexDefense) {
            const flex = lineup.flexDefense;
            if (flex.slot !== dpFlex.dpBattingSlot || flex.activePlayerId !== flexPlayerId) {
                throw new contracts_1.DiamondDomainError('invalid-dp-flex', 'FLEX history must match the linked active FLEX player.');
            }
            requireId(flex.starterPlayerId, 'FLEX starter');
            requireInteger(flex.starterReentriesUsed, 'FLEX re-entries', 0, Number.MAX_SAFE_INTEGER);
            flex.substitutions.forEach((playerId) => requireId(playerId, 'FLEX substitute'));
        }
    ...
    if (Reflect.ownKeys(state.coverage).length !== coverageFamilies.length ||
        coverageFamilies.some((family) => !Object.prototype.hasOwnProperty.call(state.coverage, family))) {
        throw new contracts_1.DiamondDomainError('invalid-coverage', 'Coverage must contain exactly the supported stat families.');
    }
    if (state.pendingIllegalPitchAwards !== undefined) {
        ...
        if (!Array.isArray(awards) ||
            ...
            awards.some((award) => !BASES.includes(award.from) ||
                state.bases[award.from]?.runnerId !== award.runnerId ||
                award.to !== (award.from === 'first' ? 'second' : award.from === 'second' ? 'third' : 'home'))) {
            throw new contracts_1.DiamondDomainError('invalid-illegal-pitch-award', 'Pending illegal-pitch awards must bind each original runner to its next base.');
        }
    }
    if (state.pendingBalkAwards !== undefined) {
        const awards = state.pendingBalkAwards;
        if (!Array.isArray(awards) ||
            awards.length > 3 ||
            (0, rules_1.requireDiamondRulesProfile)(state.rulesProfileId, state.rulesProfileVersion).sport !== 'baseball' ||
            (awards.length > 0 && Boolean(state.pendingIllegalPitchAwards?.length)) ||
            new Set(awards.map((award) => award.from)).size !== awards.length ||
            awards.some((award) => !BASES.includes(award.from) ||
                state.bases[award.from]?.runnerId !== award.runnerId ||
                award.to !== (award.from === 'first' ? 'second' : award.from === 'second' ? 'third' : 'home'))) {
            throw new contracts_1.DiamondDomainError('invalid-balk-award', 'Pending balk awards must bind each original runner to its next base.');
        }
    }
...
function reduceDiamondEvent(state, action) {
    ...
    const pendingAwardCause = state.pendingBalkAwards?.length ? 'balk' : 'illegal_pitch';
    const pendingAwards = (pendingAwardCause === 'balk' ? state.pendingBalkAwards : state.pendingIllegalPitchAwards) ?? [];
    const pendingAwardCode = pendingAwardCause === 'balk' ? 'balk-award-pending' : 'illegal-pitch-award-pending';
    if (pendingAwards.length &&
        !['advance_runner', 'private_note', 'void_event', 'supersede_event', 'suspend', 'resume', 'cancel', 'scorer_handoff'].includes(action.type)) {
        throw new contracts_1.DiamondDomainError(pendingAwardCode, 'Resolve every mandatory pitch-infraction runner award before continuing play.');
    }
    ...
    switch (action.type) {
        case 'activate': {
            ...
            if (action.payload.captureMode !== state.captureMode) {
                throw new contracts_1.DiamondDomainError('capture-mode-mismatch', 'Activation must preserve the configured ledger capture mode.');
            }
            next = {
                ...next,
                lifecycle: 'ready',
        ...
        case 'set_defensive_alignment': {
            ...
            if (state.lifecycle === 'active') {
                const profile = (0, rules_1.requireDiamondRulesProfile)(state.rulesProfileId, state.rulesProfileVersion);
                const eligibleRotation = profile.continuousBatting &&
        ...
        case 'start': {
            ...
            ['home', 'away'].forEach((side) => {
                ...
                const defendingDh = lineup.battingOrder.some((entry) => entry.battingRole === 'dh' && Object.values(lineup.defense).includes(entry.activePlayerId));
                if (remainingDefenders.length > (hasActiveDh ? 1 : 0) || (defendingDh && remainingDefenders.length > 0)) {
                    throw new contracts_1.DiamondDomainError('invalid-defensive-personnel', `${side} has a defender outside its batting order without an authorized DH or FLEX role.`);
                }
        ...
        case 'record_pitch': {
            ...
            if (action.payload.result === 'balk' &&
                (0, rules_1.requireDiamondRulesProfile)(state.rulesProfileId, state.rulesProfileVersion).sport !== 'baseball') {
                throw new contracts_1.DiamondDomainError('invalid-pitch-result', 'Balk results are only valid for baseball profiles.');
            }
            ...
            if (action.payload.result === 'balk' &&
                ...
                next = {
                    ...next,
                    pendingBalkAwards: BASES.flatMap((from) => state.bases[from]
                        ? [
                            {
                                runnerId: state.bases[from].runnerId,
                                from,
                                to: from === 'first' ? 'second' : from === 'second' ? 'third' : 'home'
                            }
                        ]
                        : [])
                };
            ...
            if (action.payload.result === 'illegal_pitch') {
                ...
                if (profile.illegalPitchPolicy === 'ball_and_advance') {
                    next = {
                        ...next,
                        pendingIllegalPitchAwards: BASES.flatMap((from) => state.bases[from]
                            ? [
                                {
                                    runnerId: state.bases[from].runnerId,
                                    from,
        ...
        case 'record_plate_appearance': {
            ...
            const completingAwardWalk = state.inning.balls === 4 &&
            ...
            if ((state.inning.balls >= 4 && result !== 'walk' && result !== 'intentional_walk') ||
                ...
                throw new contracts_1.DiamondDomainError('pitch-outcome-mismatch', 'The plate appearance must agree with its recorded terminal pitch.');
            }
            ...
            const actualOutRunnerIds = moves.flatMap((move) => (move.to === 'out' ? [move.runnerId] : []));
            const actualOutCount = actualOutRunnerIds.length;
            validateDiamondFieldingOutCredit(action.payload.fielding, actualOutCount);
            validateDiamondMergedFieldingOutCredit(action.payload.fielding ? [action.payload.fielding] : [], actualOutRunnerIds);
            ...
            const missingFullFielding = action.payload.outsOnPlay > 0 &&
                !hasCompletePutoutEvidence(action.payload.fielding ? [action.payload.fielding] : [], actualOutRunnerIds);
        ...
        case 'advance_runner': {
            ...
            if (!pendingAwards.length)
                requireOpenHalfForPlay(state);
            if (pendingAwards.length &&
                !pendingAwards.some((award) => award.runnerId === action.payload.runnerId &&
                    award.from === action.payload.from &&
                    award.to === action.payload.to &&
                    action.payload.cause === pendingAwardCause &&
            ...
            if (action.payload.fielding) {
                ...
                validateDiamondMergedFieldingOutCredit([action.payload.fielding], action.payload.to === 'out' ? [runnerId] : []);
            }
            validateOmissions(action.payload.omissions);
            ...
            next = applyMoves(next, getBattingSide(state), [
                {
                    runnerId,
            ...
            if (pendingAwards.length)
                next = {
                    ...next,
                    [pendingAwardCause === 'balk' ? 'pendingBalkAwards' : 'pendingIllegalPitchAwards']: pendingAwards.filter((award) => award.runnerId !== runnerId)
                };
            ...
            if (state.captureMode === 'full' &&
                ((action.payload.to === 'out' && !hasCompletePutoutEvidence(inlineFielding, [runnerId])) ||
        ...
        case 'add_courtesy_runner': {
            ...
            if ((profile.id === 'baseball-nfhs' || profile.id === 'fastpitch-nfhs') && courtesyRunnerIds.includes(runnerId)) {
                ...
                if (!priorRole || priorRole !== forRole) {
                    throw new contracts_1.DiamondDomainError('courtesy-runner-role-conflict', 'A courtesy runner must retain the same proven pitcher or catcher role for the entire game.');
                }
            }
            if (knownPlayerIds({ ...state.lineups[side], courtesyRunnerIds: [] }).has(runnerId)) {
                throw new contracts_1.DiamondDomainError('ineligible-courtesy-runner', 'A lineup participant cannot serve as a courtesy runner.');
            }
    ...
    if (pendingPlateAppearance &&
        ...
        throw new contracts_1.DiamondDomainError('terminal-pitch-runner-change', 'Resolve the pending plate appearance before replacing its live runners.');
    }
