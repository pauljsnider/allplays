...
The source implementation is PR #4848 at
`f941e0d058888fe936e3e13f01635516d7736087`. It is delivered as sequential,
reviewable `codex/diamond-stack-*` pull requests because the original complete
patch exceeded the reviewer's context bound. Each dependent slice remains draft
until its predecessor is merged and its exact production release and smoke are
verified. Code may be deployed while the global policy remains disabled;
rollout is controlled independently for new, explicitly activated games.
...
- [Sequential split and source reconciliation](split-reconciliation.md)
