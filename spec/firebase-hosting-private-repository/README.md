...
- Production deployment requires a first-attempt dispatch actor from the authoritative immutable-ID owner allowlist. The only production token request runs in a reusable controller invoked by full commit SHA, and the independently administered cloud provider requires its exact `job_workflow_ref` and `job_workflow_sha` plus repository/event/actor/run-attempt claims. Controller changes use an owner-approved, provider-controlled exact-SHA rotation with an unlisted-SHA denial canary. Authorization fails closed and cannot be acquired by PaulBot, another repository writer, a mutable workflow ref, a rerun, or an ordinary merge alone. Required environment-reviewer approval is optional defense in depth when the selected private plan supports it, not a GitHub Pro dependency.
