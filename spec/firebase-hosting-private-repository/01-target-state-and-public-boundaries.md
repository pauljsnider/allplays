...
4. The recommended account baseline is GitHub Pro for a private personal repository. The preflight must prove that protected branches, required checks, repository environments, environment secrets/variables, and intended collaborators are available before cutover. Required environment reviewers are not part of this baseline because GitHub does not offer that protection for private repositories on GitHub Pro; production authorization must use the provider-enforced immutable-identity design in Specification [2](./02-ci-and-deployment-architecture.md). If a later plan proves private-repository required reviewers are available, they may be added only as defense in depth.
