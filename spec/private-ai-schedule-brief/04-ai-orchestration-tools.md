...
5. Current-question and launcher scope resolution runs deterministically even if the planner omits selectors or emits a generic range such as `weekend`.
...
Add a deterministic schedule-intent classifier for obvious phrases and allow the model to produce typed candidate intent for less direct questions. Both paths enter the same temporal and identity resolver. This prevents a model from bypassing weekend normalization or launcher precedence.

### Server flow

Expose an authenticated callable or equivalent Firebase server endpoint that validates App Check where supported, derives the principal from server auth context, invokes the framework-neutral core, and returns the versioned response. A Genkit flow can provide schema binding, model/provider abstraction, and trace integration while the core owns all domain rules.
...
- [ ] Add regressions for omitted planner filters, invalid tool arguments, extra model facts, budget exhaustion, and attempted read-to-write escalation.
