...
10. App/MCP parity tests run the same fixtures through both transports and compare normalized events, ordering, coverage, truncation, absence, and deterministic text facts. MCP compatibility fixtures also prove legacy inclusive `startDate`/`endDate` inputs and defaults map to the new exclusive-instant core contract without changing their historical boundary behavior.
