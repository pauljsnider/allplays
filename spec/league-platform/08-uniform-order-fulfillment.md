...
3. Customization constraints support allowed characters, length, uniqueness rules, reserved values, and administrator review. Values configured as unique must be reserved and released through server-side transactions across confirmation, amendment, cancellation, and expiration.
...
### Customization reservations

Normalize unique values under an explicit organization, season, team, item, or other configured scope. A server transaction claims the normalized key before confirming an order line and atomically moves or releases the claim during amendments and cancellations. Expiration workers release only claims whose owning line and version still match, preventing a stale cleanup from freeing a value that has been reassigned.
...
- [ ] Implement family selection plus transactional customization reserve, confirm, amend, release, exchange, and cancellation operations.
...
- [ ] Add cutoff, duplicate import, same-value concurrency, amendment/release race, concurrent batch, partial fulfillment, and refund tests.
