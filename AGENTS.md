...
- **Canonical identity migration:** once a team has a nonempty `ownerId`, it is the only owner principal; `ownerEmail` and `ownerEmailLower` are legacy aliases, not fallback authority. Apply that precedence everywhere ownership is discovered or protected, including client/server access helpers, Firestore/Storage rules, Auth lifecycle triggers, deploy migrations, admin revocation, account deletion/scrubbing, and image-cleanup provenance. Conflicting normalized aliases fail closed before any principal binding. Search every repository use of the legacy aliases and add regressions for stale/reassigned aliases with a different canonical owner, ownerId-less legacy teams, and conflicting aliases.
