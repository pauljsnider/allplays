...
- **Cloud Function export or trigger-chain change:** `npm test` does not run every Functions loader suite. Also run `npm run test:functions:notifications`, `npm run test:functions:auth-email`, and `npm run test:functions:team-email`. Every Firebase Functions stub that loads `functions/index.js` must model the complete chain used after `runWith()`, including `runWith(...).auth.user()` for Auth triggers; keep `tests/unit/firebase-functions-test-stub-contracts.test.js` green.
- **Canonical identity migration:** once a team has a nonempty `ownerId`, it is the only owner principal; `ownerEmail` and `ownerEmailLower` are legacy aliases, not fallback authority. Apply that precedence everywhere ownership is discovered, authorized, protected, revoked, or scrubbed, including client/server access helpers, Firestore/Storage rules, Auth lifecycle triggers, deploy migrations, admin revocation, account deletion, and image-cleanup provenance. Conflicting normalized aliases fail closed in every runtime authorization/protection path, not only during principal binding or migration. Email-based authority must come from the explicit current Auth token or an enabled server Auth record; never use `email || user.email || user.profileEmail` to restore authority when the Auth email is empty, missing, disabled, or changed. Search every repository use of the legacy aliases and profile-email fallbacks, then add regressions for stale/reassigned aliases with a different canonical owner, ownerId-less legacy teams, conflicting aliases, and an empty authoritative Auth email whose stale profile fields still match an admin grant.
- **Multi-document authorization grant:** create, redeem, revoke, and migrate membership or role grants in one server-authoritative transaction that updates every reciprocal authorization record and stable principal binding together. Never pre-write a user role or membership and depend on compensating rollback after a later team/invite write. Runtime recovery must not treat mutable current email as historical grant provenance; use stable user IDs or bounded candidate-resource lifecycle evidence, fail closed when evidence is incomplete, and test rollback/response failures plus a later Auth email change.
