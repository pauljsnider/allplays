...
- When a rule removes a client write path in favor of a callable, use a compatibility rollout: deploy the callable first, keep a narrowly generated transitional rule while updated callers publish, then activate the exact server-only rule. Test the deploy order, make a partial failure safe at every boundary, and block rather than reopen permissions when the prior production state cannot be proven.
