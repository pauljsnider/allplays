...
- `parent-dashboard.html` imports `requestRideSpot` from `./js/db.js?v=27`.
- `tests/unit/parent-dashboard-calendar-day-modal-rsvp.test.js` rewrites only `./js/db.js?v=26`, so the DB import replacement misses and `requestRideSpot` is never bound in the evaluated dashboard module.
