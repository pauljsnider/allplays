...
## Root cause
`team.html` now imports `./js/db.js?v=16`, but `tests/smoke/team-schedule-calendar.spec.js` only mocks `**/js/db.js?v=15`. The smoke test therefore falls through to the real `db.js` module instead of the deterministic stub, leaving the team schedule page in its loading state under preview smoke.
