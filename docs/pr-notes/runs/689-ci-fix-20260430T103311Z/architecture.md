...
- Root cause: `team.html` imports `./js/db.js?v=17`, but the smoke harness still intercepted only `**/js/db.js?v=15`. The mock did not apply, so the page attempted to use the real DB module during static smoke and left skeleton content in place.
