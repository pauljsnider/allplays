...
## Decision
Keep the production `edit-team.html` dependency graph unchanged. The page now imports `normalizeTeamPermissions` from `./js/team-access.js?v=2` and `getAllUsers` from `./js/db.js?v=15`; those imports are required by the current page script.
