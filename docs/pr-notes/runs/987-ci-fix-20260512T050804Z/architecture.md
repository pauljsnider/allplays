...
## Root cause
The smoke test mocks are tied to stale cache-bust query strings for `auth.js` and `team-access.js`. The page imports were advanced to `auth.js?v=15` and `team-access.js?v=3`, so Playwright did not intercept the modules and the admin invite flow ran with unintended real/static behavior.
