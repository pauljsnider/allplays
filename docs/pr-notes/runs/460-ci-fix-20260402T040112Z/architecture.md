...
- Current state: `js/db.js` changed in this PR, but `index.html` still imports `./js/db.js?v=76`, so the homepage can keep serving the cached pre-fix module.
- Proposed state: bump the homepage import to `./js/db.js?v=76` so the changed `getUpcomingLiveGames` logic is cache-busted where this PR uses it.
