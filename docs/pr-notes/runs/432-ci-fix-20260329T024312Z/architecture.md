...
Current state: `js/game-day-rsvp-controls.js` assumes `loadRsvps()` must return a truthy success flag and does not guarantee a re-render after state refresh. `game-day.html` still imports `js/db.js?v=76` even though `js/db.js` changed in this PR.
