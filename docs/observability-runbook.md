...
The serialized production release train does not depend on Firebase CLI's non-persistent `projects:test` endpoint. Production rules are deployed through the documented management API: reuse an exact recent immutable ruleset when one exists, otherwise make at most three bounded `projects.rulesets.create` attempts, verify the returned single-file source byte-for-byte, update only the release's `rulesetName`, and verify that the active release resolves to the staged source. Firebase rejects a ruleset create request when the source has syntax or semantic errors; emulator-backed authorization tests run before the protected deploy as the stronger behavioral gate. API response bodies, credentials, tenant identifiers, and application data are never copied to the job summary.

Application deployment remains fail-closed. When Rules or indexes differ from the last successful production deployment, Hosting and Functions do not deploy until the exact rules release is active and a separate indexes-only Firebase CLI command succeeds. A failed attempt may have created or activated the immutable rules source without finishing indexes, so treat the Firestore configuration as potentially partial and verify both before retrying. Existing Hosting and Functions production remains active; do not bypass the workflow or deploy application surfaces separately.

To recover safely from an ambiguous create response, the workflow searches at most the 20 most recent immutable rulesets and compares the full single-file source with the staged commit-bound rules before creating again. Release activation uses a bounded idempotent `PATCH` of only `rulesetName`, followed by active-release exact-byte verification. A matching name, a transient status, ruleset creation, or a successful index deploy alone is never sufficient to continue to Hosting or Functions.
